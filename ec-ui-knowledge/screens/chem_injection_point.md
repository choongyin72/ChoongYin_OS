...
**Chem Inj Point Code*** - **Chem Inj Point Name*** - **Start Date*** (date) - Op Production Unit (first-available, grid visibility). (`*` mandatory)

### Update (`updateAttributes`) / Delete (`objectdates`)
`Chem Inj Point Code` (ro) - **`Chem Inj Point Name`**. Delete: **`End Date`** = Start Date -> leaves `OV_CHEM_INJ_POINT`.

## Automation (code in ec-automation)
- **Playwright:** `py/chem_injection_point_iud.py` (shared engine `ec_object_iud.py` + `apply_ovgm_navigator`).
- **RF:** T3 `pageobjects/Configuration/Assets/Chemical_Objects/chem_injection_point_page.resource` (**label-driven**) + suite `tests/Configuration/Assets/Chemical_Objects/chem_injection_point_iud.robot`.

...
| Purpose | Selector |
|---|---|
| Open | search `Channel` -> `label.tv-link` "Channel" |
...
**Channel Code*** - **Channel Name*** - **Start Date*** (date) - Op Production Unit (first-available, grid visibility). (`*` mandatory)

### Update (`updateAttributes`) / Delete (`objectdates`)
`Channel Code` (ro) - **`Channel Name`**. Delete: **`End Date`** = Start Date -> leaves `OV_CHANNEL`.

...
- **BF_CODE:** CD.0017 - **Treeview:** Configuration > Assets > Revenue_Split_Keys > Split Item Other _(DB treeview JSON)_
...
**Split Item Code*** - **Name*** - **Start Date*** (date) - End Date - optional dropdowns. (`*` mandatory)

### Update (`updateAttributes`) / Delete (`objectdates`)
`Split Item Code` (ro) - **`Name`**. Delete: **`End Date`** = Start Date -> leaves `OV_SPLIT_ITEM_OTHER`.

## Automation (code in ec-automation)
- **Playwright:** `py/split_item_other_iud.py` -> 7/7 (update Name).
- **RF:** T3 `pageobjects/Configuration/Assets/Revenue_Split_Keys/split_item_other_page.resource` (**label-driven, NO hardcoded ids**) + suite `tests/.../split_item_other_iud.robot` -> live 4/4.

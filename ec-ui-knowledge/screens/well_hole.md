...
- **DB view:** `OV_WELL_HOLE` (key `CODE`; `NAME`, `OBJECT_START/END_DATE`)
...
| Purpose | Selector |
|---|---|
| Open | search `Well Hole` -> `label.tv-link` "Well Hole" |
...
**Well Hole Code*** - **Well Hole Name*** - **Start Date*** (date) - Op Production Unit (first-available, grid visibility). (`*` mandatory)

### Update (`updateAttributes`) / Delete (`objectdates`)
`Well Hole Code` (ro) - **`Well Hole Name`**. Delete: **`End Date`** = Start Date -> leaves `OV_WELL_HOLE`.

## Automation (code in ec-automation)
- **Playwright:** `py/well_hole_iud.py` (shared engine `ec_object_iud.py` + `apply_ovgm_navigator`).
- **RF:** T3 `pageobjects/Configuration/Assets/Well_and_Reservoir_Objects/well_hole_page.resource` (**label-driven**) + suite `tests/Configuration/Assets/Well_and_Reservoir_Objects/well_hole_iud.robot`.

...
with sync_playwright() as p:
    ...
    for label in ["Delivery Point Name","Delivery Stream Name"]:
        r=ec._resolve_field(pg,"objectForm",label)
        ok = bool(r and r["kind"]=="popup")
        print(f"resolve '{label}' -> {r} | detected popup: {ok}")
        if ok:
            try:
                ec.pick_popup(pg, r["id"], "__FIRST__")
                val=pg.eval_on_selector(ec._css(r["id"]),"e=>e.value")
                print(f"   picked -> pin value: {val!r}  => {'PASS' if val and val.strip() else 'EMPTY(source?)'}")
            except Exception as e:
                print(f"   pick error: {repr(e)[:120]}")
    b.close()

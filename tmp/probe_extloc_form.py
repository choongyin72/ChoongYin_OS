#!/usr/bin/env python3
"""READ-ONLY: External Location's New-Object form was never reached by the earlier scan (no mandatory
dropdown to trigger the gated-nav path). Click GO (go_only mode), then open New Object, and dump the
mandatory fields before any live insert is attempted."""
import os, sys
...
with sync_playwright() as p:
    ...
    print(a("mandatory objectForm fields:"))
    for f in fields:
        if f["mandatory"]:
            print(a("   %-56s %s" % (f["id"], f["label"])))
    print(a("\nALL objectForm fields (for context):"))
    for f in fields:

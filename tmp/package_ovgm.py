...
if ("py/%s_iud.py" % slug) not in reg_txt and ("| %s |" % screen) not in reg_txt:
    ...
    row = ("| %s | %s > %s (%s) | OV-GM (manage-object, groupmodel) verify_screen PASS %s - RF %s + %s, "
           "DB-verified, self-clean; label-driven | `%s` (versioned) | %s cascade + GO | End Date = Start Date "
           "| `manageObject:form:T_data` | `pageobjects/%s/%s_page.resource`; driver `py/%s_iud.py` (mandatory %s/%s/Start Date%s) |\n"
           % (screen, tv, screen, bf, date, rf_txt, pw_txt, view, " -> ".join(nav),
              folder, slug, slug, code_l, name_l, dd_r))

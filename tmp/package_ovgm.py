...
''' % dict(screen=screen, bf=bf, tv=tv, view=view, date=date, nav=" -> ".join(nav),
           ...
           fam_type=(FAM_KB_TYPE[family].replace(
                         "navigator-GATED.", "GO only - NO mandatory nav scope (fields are optional filters).")
                     if nav_mode == "go_only" else FAM_KB_TYPE[family]),

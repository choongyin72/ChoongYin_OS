#!/usr/bin/env python3
"""READ-ONLY: with Start Date=2020-01-01 and Contract Name='TS5 Shipper B Firm' selected, does Location
Name populate (cascade child)? Nothing saved."""
import os, sys
...
with sync_playwright() as p:
    ...
    ec.select_dropdown(pg, cf["id"], "TS5 Shipper B Firm")
    pg.wait_for_timeout(1200)
    print(a("Contract Name set"))
    ...
    opts = pg.eval_on_selector_all('[id="%s_panel"] tr[data-item-label]' % lbase,
                                   "els => els.map(e => e.dataset.itemLabel)")
    print(a("Location Name after Contract set: %d options: %s" % (len(opts), opts[:10])))
    br.close()

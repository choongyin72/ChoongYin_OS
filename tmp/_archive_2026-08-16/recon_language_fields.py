"""Read-only recon: open Language, capture grid columns + toolbar flyout text, before writing the
canonical engine regression canary. No Save, no data entered."""
import sys
...
with sync_playwright() as p:
    ...
    inv = eng.field_inventory(grid_id=grid_id)
    import json
    print(json.dumps(inv, indent=2))

    rows = page.evaluate(
        """(gid) => { const tb = document.getElementById(gid);
        return Array.from(tb.querySelectorAll('tr[data-ri]')).slice(0, 5).map(tr => ({

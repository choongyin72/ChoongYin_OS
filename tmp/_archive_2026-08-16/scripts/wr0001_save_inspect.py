"""Inspect ALL Save controls on WR.0001 after a real-keystroke edit (READ-ONLY apart from staging
one edit; does NOT save). Reveals which Save anchor to click + its full onclick (s/f/p/u)."""
import time, json
...
with sync_playwright() as p:
    ...
    # real-keystroke edit to dirty the form (so Save enables) — value 27 (unique-ish)
    el=fr.locator(f'[id="{CELL}"]'); el.click(); el.press("Control+a"); el.press("Delete"); el.type("27", delay=80); el.press("Tab")
    page.wait_for_load_state("networkidle",timeout=12000); time.sleep(1.8)
    print("cell:", el.input_value())
    # dump every Save anchor anywhere (top doc == main frame here)
    saves = page.evaluate("""()=>[...document.querySelectorAll('a[title^=\"Save\"], button[title^=\"Save\"]')].map((e,i)=>{
        const r=e.getBoundingClientRect();
        return {i, tag:e.tagName, title:e.title, vis:e.offsetParent!==null, disabled:e.className.includes('ui-state-disabled'),
                w:Math.round(r.width), h:Math.round(r.height), x:Math.round(r.x), y:Math.round(r.y),
                cls:e.className, onclick:(e.getAttribute('onclick')||''), parent:(e.parentElement?e.parentElement.id||e.parentElement.className:'')};
    })""")
    print("\n=== ALL Save anchors ("+str(len(saves))+") ===")
    for s in saves: print(json.dumps(s))
    b.close()
print("DONE")

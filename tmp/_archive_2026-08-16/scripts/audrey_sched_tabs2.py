...
scan grid rows (with pagination) for AudreyExcelImport -> open BUSINESS ACTION + SCHEDULE tabs."""
import os
import time
...
with sync_playwright() as p:
    ...
    if found:
        page.locator('xpath=//tbody[@id="schedule:form:T_data"]//input[@value="AudreyExcelImport"]').first.click()
        page.wait_for_load_state("networkidle", timeout=15000)
        time.sleep(1.5)
        ...
        for tab in ["BUSINESS ACTION", "SCHEDULE"]:
            ...
            if t.count():
                ...
                page.wait_for_load_state("networkidle", timeout=15000)
                time.sleep(2)
                page.screenshot(path=str(OUT / f"audrey_sched_{tab.lower().replace(' ', '_')}.png"), full_page=True)
                print("captured", tab)
    browser.close()

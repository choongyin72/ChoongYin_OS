"""URGENT self-clean #2: the C2 probe's revert didn't fire, leaving Methane WT_PCT(C1)=0.2 (orig 0.1).
Fresh-session restore: navigate PO.0019 grid -> set Methane C2_in (wt%) = 0.1 -> Save (robust) ->
verify DB WT_PCT(C1)=0.1. (Fresh-session edit+save is reliable; the revert-in-same-session is what failed.)"""
import os
import oracledb
...
print("DB before restore: WT_PCT(C1) =", wt_c1())
with sync_playwright() as p:
    ...
    print("Methane wt% cell:", cid, "val:", page.locator(css(cid)).input_value())
    inp = page.locator(css(cid)); inp.click()
    page.keyboard.press("Control+A"); page.keyboard.press("Delete"); page.wait_for_timeout(150)
    page.keyboard.type("0.1", delay=80); page.keyboard.press("Tab"); ajax(page, 14000)
    ...
    try:
        ...
        print("save not enabled:", str(e)[:60])
    b.close()
print("DB after restore: WT_PCT(C1) =", wt_c1(), " (must be 0.1)")

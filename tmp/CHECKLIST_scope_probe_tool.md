...
- [x] The SAME defect recurred THREE times as three separate live-debugging sessions: Message Group
      (functional area saved wrong), Service (contract/transport system saved wrong), Collection Point
      (first-available PU's cascade children empty). Confirmed by re-reading each park/build record.
- [x] The only artifact produced after Service was a DOC PARAGRAPH (`ov-gm-navigator-capability.md`) - a
      thing I could read, not a thing that forces me to check. I was about to repeat the live-debug cycle a
      third time on Collection Point when stopped. Same failure shape as the `check_known_issue.py` lesson,
...
## Proven against all 3 known failure cases, in ONE command each
- [x] `OV_MESSAGE_GROUP` -> `FUNCTIONAL_AREA_CODE [('EC',2),('MHM13_PROD',2)]` - would have shown
      immediately that `Administration` was never a used value, before any live attempt.

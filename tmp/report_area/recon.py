"""READ-ONLY recon for Report Area (RP.0017, OV_REPORT_AREA): DB columns + treeview path +
...
cur.execute("""select column_name,data_type,nullable from all_tab_columns
               where table_name='OV_REPORT_AREA' order by column_id""")
print("=== OV_REPORT_AREA columns ===")
for c in cur.fetchall(): print(f"  {c[0]:<22} {c[1]:<10} null={c[2]}")
# --- treeview path (screen RP.0017) ---
...
def walk(n,path):
    ...
    if n.get("screen")=="RP.0017": hits.append(np)
    for c in n.get("children",[]) or []:
        if isinstance(c,dict): walk(c,np)
for r in data["configuration"]["items"]: walk(r,[])
print("\nTREEVIEW PATH (RP.0017):", " > ".join(hits[0]) if hits else "NOT FOUND")
...
with sync_playwright() as p:
    ...
    print("\nscreen:", ec.open_object_screen(pg,"Report Area"))
    ec.click_go(pg)

...
- ✅✅ **Var B2 (the READ MAPPING) — PROVEN 2026-06-30** (binds a variable to a `_DATA` class = the item-1 data-model rule, hands-on). `AUTOTEST_rCO2Rate` → `CALC_VAR_READ_MAPPING` (cls=PWEL_DAY_DATA, attr=PREC_THEOR_CO2_RATE). Two gestures cracked it (`investigation/phase1b_create_varB2_v2.py`):
  1. **MASTER-SELECT the variable: click its `C0_in` then press `Escape`** → the row goes `ui-state-highlight` (selected, not editing). *Without this the CLASS READ MAPPING insert is a disabled no-op.* (Confirmed via `phase1b_b2_diag3.py`: menuitem `disabled:false` only after proper select.)
  2. The new mapping row cells are **type-ahead autocomplete dropdowns** (`C1_dd`=Class Type, `C2_dd`=Class Name, `C4_dd`=Value Attribute; `C0_dd`=Data Set defaults "Default"; C3/C5=derived labels) — **type the needle into `_dd_input`**, wait, then click the matching option (button-open shows no static list). Set order: Class Type=Data → Class Name=PWEL_DAY_DATA → Value Attribute.
  - `CLASS KEY READ MAPPING` (the dimension-key map) is **disabled until a class mapping row exists**; the read-mapping row persists with just class+attribute. Deleting the variable **cascades** its read mapping (verified clean).
  - ⚠️ refinement: my type-ahead `CO2_RATE` matched `PREC_THEOR_CO2_RATE` (precision variant) before `THEOR_CO2_RATE` — for an exact attribute, match the full code.
  - Only ONE insert toolbar (top `screenToolbar:form:menuBar`); its flyout lists VARIABLE DEFINITION + the mapping types (no separate sub-grid toolbar).

...
- ⚠️ **Var B2 (the READ MAPPING) — OPEN BLOCKER (1 attempt, stopped per ≤2-try rule).** Structure + exact clone spec are known: a full read mapping = **CLASS READ MAPPING** row (Data Set=Default, Class Type=Data, **Class Name=PWEL_DAY_DATA**, **Value Attribute=THEOR_CO2_RATE**) **+ two CLASS KEY READ MAPPING** rows (OBJECT_ID→Dimension 1, DAYTIME→Dimension 2) — all added via the **insert flyout** (`CLASS READ MAPPING` / `CLASS KEY READ MAPPING`). **Blocker:** after inserting CLASS READ MAPPING, the `readMapping:form:T_data` grid showed a **pre-existing/placeholder row** and my empty-row detector (`C2_in` empty) returned **-1**, so no cell got filled and nothing persisted (`CALC_VAR_READ_MAPPING` empty). **Next:** dump the readMapping grid's full row state immediately after the flyout insert (which row index is the new editable one; is C2 a free-text class field that must resolve via Tab, or a lookup) before filling. Script `investigation/phase1b_create_varB2_mapping.py`.

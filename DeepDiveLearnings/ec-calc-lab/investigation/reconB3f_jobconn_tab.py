"""SCAN (HEADED, read-only): open the CALCULATION JOB CONNECTION tab -> dump its grid (existing job rows) +
whether the Insert>'Calculation Job' sub-item is now enabled. No add/save."""
...
with sync_playwright() as p:
    ...
    # click the CALCULATION JOB CONNECTION tab (case-insensitive text)
    tab=pg.locator("xpath=//a[contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'job connection')]")
    print("JOB CONNECTION tab count:",tab.count())
    if tab.count()==0: print("ABORT: tab not found"); b.close(); raise SystemExit
    tab.first.click(); wa(pg)
    pg.screenshot(path=os.path.join(SS,'buildB_08_jobconn.png'))
    info=pg.evaluate("""()=>{
      const tbs=[...document.querySelectorAll("tbody[id$='_data']")].filter(t=>t.offsetParent!==null && (/calculation test|daily well/i.test(t.innerText) || /job|conn/i.test(t.id))).map(t=>({id:t.id, txt:t.innerText.replace(/\s+/g,' ').slice(0,70)}));
      const cj=[...document.querySelectorAll("a")].filter(e=>/^Calculation Job$/.test((e.textContent||'').trim())).map(e=>({vis:e.offsetParent!==null, disabled:/disabled/.test(e.className||''), id:e.id||''}));
      return {tbs, cj};
    }""")
    print("job-conn grid(s):",info['tbs'])
    print("'Calculation Job' insert items:",info['cj'])

...
5. ☐ **`REV_TEXT`** set on **every** INSERT and UPDATE to a *real, meaningful* value: the governing `ECPR-NNNNN` for client work, or `'ECPR-DEMO'` for sandbox/demo objects with no client ticket. **Never ship the literal `'ECPR-XXXX'` placeholder** in a script that will run (R22) — `grep ECPR-XXXX *.sql` before push and replace any hit.

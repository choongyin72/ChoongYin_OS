...
- **Protected attribute**: Insert throws "Not allowed to insert protected attributes." — dismiss via
  ...
  a case-insensitive selector). This is a real business rule (see §1), not a bug — never retry.
...
- For anything beyond a quick check, also DB-verify via `DV_CONTRACT_ATTRIBUTE` (see §1).

...
| v38 | R38 | Conversion/rebuild PR updates the FULL doc set (registry + scorecard + BOTH checklists) in the same diff | 2026-08-24 |
...
## 2026-07-13 01:00 AWST — Automated Review (off-schedule/early run, 1 open PR #178, STANDING/DRAFT)

_Off-schedule early run (01:10 AWST; branch/log labelled `01` to avoid collision with a later real 06:00 run). Open PR triggers a full review (R14). #178's head `56ed452` is byte-identical to the commit cleared at #180-#184 and master has not advanced (0 new commits since #184/`48cc762`) — an unchanged-head **RE-CONFIRMATION (6th consecutive on `56ed452`)**, NOT a re-review of frozen content. **Re-confirmed CLEAR; NOT merged** (owner-merge-only standing draft, still `draft:true`). **No new executable rules — version stays v29.** R1-R29 remain current._

### PR Status after this review pass

| PR | Finding | Status |
|----|---------|--------|
| #178 | Clear (re-confirmation). Head `56ed452` unchanged since the 07-10 06:00 review (#180) and re-confirmed at #181/#182/#183/#184; `origin/feature/ec-screen-deepdive` = `56ed452`, master unchanged since `48cc762` → no new pushed content to re-litigate. Safety invariants re-verified at the tip (MR1, checked not inferred): **R23 CLEAN** (three-dot `git diff --numstat origin/master...origin/feature/ec-screen-deepdive` on all four reviewer-owned docs = zero `-` lines); **R28 PASS** (CLAUDE.md 124 lines, < 200); never-auto-merge holds. **R27 pre-milestone advisory re-confirmed** (owner action, not reviewer): two-dot `git diff origin/master origin/feature/ec-screen-deepdive` shows the branch behind master on `docs/lessons-learned.md -148` + `docs/review-log.md -6` + `STATUS.md` (= the #179-#184 review entries not re-absorbed) — the literal R23 test is the THREE-dot form and it is CLEAN, so NOT an R23 MUST-FIX, but the owner MUST `git merge origin/master` (or `--no-ff`, NOT squash per R27) before the next milestone push/merge or a squash would clobber those review records. **R29** — no new content pushed this cycle, so no stale-runner artifact-format mismatch to flag. **NOT merged** (owner-merge-only standing draft). | ✅ Clear — left open (owner-merge-only) |

### Observations (good patterns to keep)

- **An unchanged open-PR head is a re-confirmation, not a re-review — and that is the correct, cheap shape for a standing-draft the reviewer is forbidden to merge.** Six consecutive runs (#180 through this one) have found `56ed452` byte-identical; each verified the three safety invariants (never-auto-merge, R23 three-dot clean, R28 size gate) and the R27 pre-milestone advisory rather than re-scoring already-cleared content. Confirm the invariants, don't re-litigate the diff.
- **R23 (three-dot) vs R27 (two-dot) stay correctly distinguished.** The branch is legitimately behind master on the reviewer-owned docs (two-dot shows `-148`/`-6`), but the authoritative clobber-risk test is the three-dot merge-base diff, which is empty — so this is an owner pre-milestone-merge action (R27), never an R23 MUST-FIX. Continuing to report both forms keeps the distinction honest across the long re-confirmation streak.

### Gaps (verified against filesystem / GitHub API)

| Gap | Owner | Priority |
|-----|-------|----------|
| **R27 owner action (pre-milestone):** before the next milestone merge of `feature/ec-screen-deepdive`, `git merge origin/master` (or `--no-ff`, NOT squash) so the #179-#184 reviewer-doc entries are re-absorbed — a squash as-is would clobber them (R23 three-dot is clean today, but only because the branch simply does not edit those files; a milestone squash still needs the re-absorb) | Owner | 🟡 Medium |
| Carry-over (still open): extend `check_bundle_hygiene.py` ASCII gate to `.claude/skills/**/*.py` + `workstreams/**/scripts/*.py` + `tools/**`; `DeepDiveLearnings/ec-screens/gen_checklist.py:33` em-dash `print` (R18/R20); `ec-sql-script-builder` demo SQL `REV_TEXT='ECPR-XXXX'` -> `'ECPR-DEMO'` (R22); `EXECUTION-PLAN-completion.md` Phase 4 still prescribes squash (R27 — switch to `--no-ff`/rebase); R9 header drift on the bot-generated draft body (harmless for a never-auto-merged draft) | Worker | 🟡 Medium |

### Reviewer process note (isolated worktree; stale main-checkout docs; off-schedule label)

- The main checkout (`C:\Projects\ChoongYin_OS`) is the Worker's permanent branch `feature/ec-screen-deepdive` at a stale tip (`a7a0a3d`, ~100 commits behind master), so the session-start mandatory-read `docs/lessons-learned.md`/`docs/review-log.md` there were the STALE **v23** copies; the live **v29** docs were re-read from the `origin/master` copy in the worktree before any rule/version decision (MR4). ALL review-doc edits were made in an isolated `C:/tmp/wt-review-2026-07-13-01` worktree off `origin/master`; the Worker's dirty permanent-branch checkout and all sibling `wt-*` worktrees (incl. the runner's `wt-ec-learn`) were never touched. Steps 4b/18's `git checkout master` in the main checkout would disrupt the parallel Worker session and were deliberately NOT run there. Run fired off-schedule at 01:10 AWST (neither 06:00 nor 14:00); labelled `01` in the branch name + review-log so a later real 06:00 run neither collides on the branch nor is suppressed by the review-log dedup guard.

---

## 2026-07-14 06:00 AWST — Automated Review (0 new master commits since #186; 1 open PR #178, STANDING/DRAFT)

_Open PR triggers a full review (R14). #178's head `56ed452` is byte-identical to the commit cleared at #180 through #186 and master has not advanced (0 new commits since #186/`ebdb2d1`) — an unchanged-head **RE-CONFIRMATION (8th consecutive on `56ed452`)**, NOT a re-review of frozen content. **Re-confirmed CLEAR; NOT merged** (owner-merge-only standing draft, still `draft:true`). **No new executable rules — version stays v29.** R1-R29 remain current._

### PR Status after this review pass

| PR | Finding | Status |
|----|---------|--------|
| #178 | Clear (re-confirmation). Head `56ed452` unchanged since the 07-10 06:00 review (#180) and re-confirmed at #181/#182/#183/#184/#185/#186; `gh pr view 178` → `headRefOid=56ed452`, `isDraft:true`, master unchanged since `ebdb2d1` → no new pushed content to re-litigate. Safety invariants re-verified at the tip (MR1, checked not inferred): **R23 CLEAN** (three-dot `git diff --numstat origin/master...origin/feature/ec-screen-deepdive` on all four reviewer-owned docs = EMPTY, zero `-` lines); **R28 PASS** (CLAUDE.md 124 lines, < 200); never-auto-merge holds. **R27 pre-milestone advisory re-confirmed** (owner action, not reviewer): two-dot `git diff --stat origin/master origin/feature/ec-screen-deepdive` shows the branch behind master on `docs/lessons-learned.md -176` + `docs/review-log.md -8` + `STATUS.md` (= the #179-#186 review entries not re-absorbed) — the literal R23 test is the THREE-dot form and it is CLEAN, so NOT an R23 MUST-FIX, but the owner MUST `git merge origin/master` (or `--no-ff`, NOT squash per R27) before the next milestone push/merge or a squash would clobber those review records. **R29** — no new content pushed this cycle, so no stale-runner artifact-format mismatch to flag. **NOT merged** (owner-merge-only standing draft). | ✅ Clear — left open (owner-merge-only) |

### Observations (good patterns to keep)

- **An unchanged open-PR head is a re-confirmation, not a re-review — the correct, cheap shape for a standing-draft the reviewer is forbidden to merge.** Eight consecutive runs (#180 through this one) have found `56ed452` byte-identical; each verified the three safety invariants (never-auto-merge, R23 three-dot clean, R28 size gate) and the R27 pre-milestone advisory rather than re-scoring already-cleared content. Confirm the invariants, don't re-litigate the diff. The two open code-hygiene NICE-TO-HAVEs were re-checked directly at the tip (MR1), not assumed: `run_ec_screen_learn.py:104` still has the dead 7th resolver candidate (`re.sub(...,r'',base)` → always empty, filtered by `if c`), and `gen_checklist.py:33` still emits a U+2014 em-dash to stdout (`od -c` → `342 200 224`).
- **R23 (three-dot) vs R27 (two-dot) stay correctly distinguished across the long streak.** The branch is legitimately behind master on the reviewer-owned docs (two-dot shows `-176`/`-8`, grown by one more review entry since #185's `-148`/`-6`), but the authoritative clobber-risk test is the three-dot merge-base diff, which is empty — so this is an owner pre-milestone-merge action (R27), never an R23 MUST-FIX. Reporting both forms every run keeps the distinction honest as the drift accumulates.

### Gaps (verified against filesystem / GitHub API)

| Gap | Owner | Priority |
|-----|-------|----------|
| **R27 owner action (pre-milestone):** before the next milestone merge of `feature/ec-screen-deepdive`, `git merge origin/master` (or `--no-ff`, NOT squash) so the #179-#186 reviewer-doc entries are re-absorbed — a squash as-is would clobber the accumulated review records (R23 three-dot is clean today only because the branch does not edit those files; a milestone squash still needs the re-absorb) | Owner | 🟡 Medium |
| Resolver dead 7th candidate `run_ec_screen_learn.py:104` — `re.sub(...,r'',base)` always empty (intended `r'\2'`); carry-over from #170 onward, verified still at tip `56ed452` | Worker | 🟢 Low |
| Carry-over (still open): extend `check_bundle_hygiene.py` ASCII gate to `.claude/skills/**/*.py` + `workstreams/**/scripts/*.py` + `tools/**`; `DeepDiveLearnings/ec-screens/gen_checklist.py:33` em-dash `print` (R18/R20, verified still present); `ec-sql-script-builder` demo SQL `REV_TEXT='ECPR-XXXX'` → `'ECPR-DEMO'` (R22); `EXECUTION-PLAN-completion.md` Phase 4 still prescribes squash (R27 — switch to `--no-ff`/rebase); R9 header drift on the bot-generated draft body (harmless for a never-auto-merged draft); ECIS `upload -> RUN NOW` flakiness root cause; Reported Alarms EVENT_LOG clone; WR.0010.02 Well Oil Comp automation | Worker | 🟡 Medium |

### Reviewer process note (isolated worktree; stale main-checkout docs)

- The main checkout (`C:\Projects\ChoongYin_OS`) is the Worker's permanent branch `feature/ec-screen-deepdive` at a stale tip (`a7a0a3d`, **103 commits behind master**), so the session-start mandatory-read `docs/lessons-learned.md`/`docs/review-log.md` there were the STALE **v23** copies; the live **v29** docs (R1-R29) were re-read from the `origin/master` copy in the worktree before any rule/version decision (MR4). ALL review-doc edits were made in an isolated `C:/tmp/wt-review-2026-07-14-0600` worktree off `origin/master`; the Worker's dirty permanent-branch checkout and all sibling `wt-*` worktrees (incl. the runner's `wt-ec-learn`) were never touched. Steps 4b/18's `git checkout master` in the main checkout would disrupt the parallel Worker session and were deliberately NOT run there.

---

## 2026-07-15 06:00 AWST — Automated Review (0 new master commits since #187; 1 open PR #178, STANDING/DRAFT)

_Open PR triggers a full review (R14). #178's head `56ed452` is byte-identical to the commit cleared at #180 through #187 and master has not advanced (0 new commits since #187/`a580d76`) — an unchanged-head **RE-CONFIRMATION (9th consecutive on `56ed452`)**, NOT a re-review of frozen content. **Re-confirmed CLEAR; NOT merged** (owner-merge-only standing draft, still `draft:true`). **No new executable rules — version stays v29.** R1-R29 remain current._

### PR Status after this review pass

| PR | Finding | Status |
|----|---------|--------|
| #178 | Clear (re-confirmation). Head `56ed452` unchanged since the 07-10 06:00 review (#180) and re-confirmed at #181-#187; `gh pr view 178` → `headRefOid=56ed452`, `isDraft:true`, master unchanged since `a580d76` → no new pushed content to re-litigate. Safety invariants re-verified at the tip (MR1, checked not inferred): **R23 CLEAN** (three-dot `git diff --numstat origin/master...origin/feature/ec-screen-deepdive` on all four reviewer-owned docs = EMPTY, zero `-` lines); **R28 PASS** (CLAUDE.md 124 lines, < 200); never-auto-merge holds. **R27 pre-milestone advisory re-confirmed** (owner action, not reviewer): two-dot `git diff --stat origin/master origin/feature/ec-screen-deepdive` shows the branch behind master on `docs/lessons-learned.md -205` + `docs/review-log.md -9` + `STATUS.md` (= the #179-#187 review entries not re-absorbed) — the literal R23 test is the THREE-dot form and it is CLEAN, so NOT an R23 MUST-FIX, but the owner MUST `git merge origin/master` (or `--no-ff`, NOT squash per R27) before the next milestone push/merge or a squash would clobber those review records. **R29** — no new content pushed this cycle, so no stale-runner artifact-format mismatch to flag. **NOT merged** (owner-merge-only standing draft). | ✅ Clear — left open (owner-merge-only) |

### Observations (good patterns to keep)

- **Nine consecutive re-confirmations on a frozen head is the correct, cheap shape for a standing-draft the reviewer is forbidden to merge.** Runs #180 through this one have found `56ed452` byte-identical every time; each verifies the three safety invariants (never-auto-merge, R23 three-dot clean, R28 size gate) plus the R27 pre-milestone advisory rather than re-scoring already-cleared content. Confirm the invariants, don't re-litigate the diff.
- **R23 (three-dot) vs R27 (two-dot) stay correctly distinguished as the drift grows.** The branch is legitimately behind master on the reviewer-owned docs (two-dot now `-205`/`-9`, grown by one more review entry since #187's `-205`/`-8` window) but the authoritative clobber-risk test is the three-dot merge-base diff, which is empty — so this remains an owner pre-milestone-merge action (R27), never an R23 MUST-FIX. Reporting both forms every run keeps the distinction honest.
- **Both open code-hygiene NICE-TO-HAVEs re-checked directly at the tip (MR1), not assumed:** `run_ec_screen_learn.py:104` still carries the dead 7th resolver candidate (`re.sub(...,r'',base)` → always empty, filtered by `if c`), and `gen_checklist.py:33` still emits a U+2014 em-dash to stdout (`od -c` → `342 200 224`). Both non-gating carry-overs, unchanged since the head froze.

### Gaps (verified against filesystem / GitHub API)

| Gap | Owner | Priority |
|-----|-------|----------|
| **R27 owner action (pre-milestone):** before the next milestone merge of `feature/ec-screen-deepdive`, `git merge origin/master` (or `--no-ff`, NOT squash) so the #179-#187 reviewer-doc entries are re-absorbed — a squash as-is would clobber the accumulated review records (R23 three-dot is clean today only because the branch does not edit those files; a milestone squash still needs the re-absorb) | Owner | 🟡 Medium |
| Resolver dead 7th candidate `run_ec_screen_learn.py:104` — `re.sub(...,r'',base)` always empty (intended `r'\2'`); carry-over from #170 onward, verified still at tip `56ed452` | Worker | 🟢 Low |
| Carry-over (still open): extend `check_bundle_hygiene.py` ASCII gate to `.claude/skills/**/*.py` + `workstreams/**/scripts/*.py` + `tools/**`; `DeepDiveLearnings/ec-screens/gen_checklist.py:33` em-dash `print` (R18/R20, verified still present); `ec-sql-script-builder` demo SQL `REV_TEXT='ECPR-XXXX'` → `'ECPR-DEMO'` (R22); `EXECUTION-PLAN-completion.md` Phase 4 still prescribes squash (R27 — switch to `--no-ff`/rebase); R9 header drift on the bot-generated draft body (harmless for a never-auto-merged draft); ECIS `upload -> RUN NOW` flakiness root cause; Reported Alarms EVENT_LOG clone; WR.0010.02 Well Oil Comp automation | Worker | 🟡 Medium |
...
## 2026-07-16 00:00 AWST — Automated Review (off-schedule run, 0 new master commits since #188; 1 open PR #178, STANDING/DRAFT)

_Off-schedule run (00:00 AWST; branch/log labelled `00` to avoid collision with a later real 06:00 run, per the 2026-07-13 01:00 precedent). Open PR triggers a full review (R14). #178's head `56ed452` is byte-identical to the commit cleared at #180 through #188 and master has not advanced (0 new commits since #188/`5e7986c`) — an unchanged-head **RE-CONFIRMATION (10th consecutive on `56ed452`)**, NOT a re-review of frozen content. **Re-confirmed CLEAR; NOT merged** (owner-merge-only standing draft, still `draft:true`). **No new executable rules — version stays v29.** R1-R29 remain current._

### PR Status after this review pass

| PR | Finding | Status |
|----|---------|--------|
| #178 | Clear (re-confirmation). Head `56ed452` unchanged since the 07-10 06:00 review (#180) and re-confirmed at #181-#188; `pull_request_read` → `head.sha=56ed452`, `draft:true`, `merged:false`, master unchanged since `5e7986c` → no new pushed content to re-litigate. Safety invariants re-verified at the tip (MR1, checked not inferred): **R23 CLEAN** (three-dot `git diff --numstat origin/master...origin/feature/ec-screen-deepdive` on all four reviewer-owned docs = EMPTY, zero `-` lines); **R28 PASS** (CLAUDE.md 124 lines, < 200); never-auto-merge holds. **R27 pre-milestone advisory re-confirmed** (owner action, not reviewer): two-dot `git diff --stat origin/master origin/feature/ec-screen-deepdive` shows the branch behind master on `docs/lessons-learned.md -235` + `docs/review-log.md -10` + `STATUS.md` (= the #179-#188 review entries not re-absorbed) — the literal R23 test is the THREE-dot form and it is CLEAN, so NOT an R23 MUST-FIX, but the owner MUST `git merge origin/master` (or `--no-ff`, NOT squash per R27) before the next milestone push/merge or a squash would clobber those review records. **R29** — no new content pushed this cycle, so no stale-runner artifact-format mismatch to flag. **NOT merged** (owner-merge-only standing draft). | ✅ Clear — left open (owner-merge-only) |

### Observations (good patterns to keep)

- **Ten consecutive re-confirmations on a frozen head is the correct, cheap shape for a standing-draft the reviewer is forbidden to merge.** Runs #180 through this one have found `56ed452` byte-identical every time; each verifies the three safety invariants (never-auto-merge, R23 three-dot clean, R28 size gate) plus the R27 pre-milestone advisory rather than re-scoring already-cleared content. Confirm the invariants, don't re-litigate the diff.
- **R23 (three-dot) vs R27 (two-dot) stay correctly distinguished as the drift grows.** The branch is legitimately behind master on the reviewer-owned docs (two-dot now `-235`/`-10`, grown by one more review entry since #188's `-205`/`-9`) but the authoritative clobber-risk test is the three-dot merge-base diff, which is empty — so this remains an owner pre-milestone-merge action (R27), never an R23 MUST-FIX. Reporting both forms every run keeps the distinction honest.
- **Both open code-hygiene NICE-TO-HAVEs re-checked directly at the tip (MR1), not assumed:** `run_ec_screen_learn.py:104` still carries the dead 7th resolver candidate (`re.sub(...,r'',base)` → always empty, filtered by `if c`), and `gen_checklist.py:33` still emits a U+2014 em-dash to stdout (Python `ord()>127` scan → `0x2014`; a Windows `grep -P "[^\x00-\x7F]"` misses it, so the Python scan is authoritative on this platform). The runner itself re-scanned 0 non-ASCII bytes (R18/R20). Both non-gating carry-overs, unchanged since the head froze.

### Gaps (verified against filesystem / GitHub API)

| Gap | Owner | Priority |
|-----|-------|----------|
| **R27 owner action (pre-milestone):** before the next milestone merge of `feature/ec-screen-deepdive`, `git merge origin/master` (or `--no-ff`, NOT squash) so the #179-#188 reviewer-doc entries are re-absorbed — a squash as-is would clobber the accumulated review records (R23 three-dot is clean today only because the branch does not edit those files; a milestone squash still needs the re-absorb) | Owner | 🟡 Medium |
| Resolver dead 7th candidate `run_ec_screen_learn.py:104` — `re.sub(...,r'',base)` always empty (intended `r'\2'`); carry-over from #170 onward, verified still at tip `56ed452` | Worker | 🟢 Low |
| Carry-over (still open): extend `check_bundle_hygiene.py` ASCII gate to `.claude/skills/**/*.py` + `workstreams/**/scripts/*.py` + `tools/**`; `DeepDiveLearnings/ec-screens/gen_checklist.py:33` em-dash `print` (R18/R20, verified still present via Python ord-scan); `ec-sql-script-builder` demo SQL `REV_TEXT='ECPR-XXXX'` → `'ECPR-DEMO'` (R22); `EXECUTION-PLAN-completion.md` Phase 4 still prescribes squash (R27 — switch to `--no-ff`/rebase); R9 header drift on the bot-generated draft body (harmless for a never-auto-merged draft); ECIS `upload -> RUN NOW` flakiness root cause; Reported Alarms EVENT_LOG clone; WR.0010.02 Well Oil Comp automation | Worker | 🟡 Medium |

### Reviewer process note (isolated worktree; stale main-checkout docs; off-schedule label)

- The main checkout (`C:\Projects\ChoongYin_OS`) is the Worker's permanent branch `feature/ec-screen-deepdive` at a stale tip (`a7a0a3d`, ~104 commits behind master), so the session-start mandatory-read `docs/lessons-learned.md`/`docs/review-log.md` there were the STALE **v23** copies; the live **v29** docs (R1-R29) were re-read from the `origin/master` copy in the worktree before any rule/version decision (MR4). ALL review-doc edits were made in an isolated `C:/tmp/wt-review-2026-07-16-0000` worktree off `origin/master`; the Worker's dirty permanent-branch checkout and all sibling `wt-*` worktrees (incl. the runner's `wt-ec-learn`) were never touched. Steps 4b/18's `git checkout master` in the main checkout would disrupt the parallel Worker session and were deliberately NOT run there. Run fired off-schedule at 00:00 AWST (neither 06:00 nor 14:00); labelled `00` in the branch name + review-log so a later real 06:00 run neither collides on the branch nor is suppressed by the review-log dedup guard.

---

## 2026-07-16 06:00 AWST — Automated Review (0 new master commits since #189; 1 open PR #178, STANDING/DRAFT)

_Open PR triggers a full review (R14). #178's head `56ed452` is byte-identical to the commit cleared at #180 through #189 and master has not advanced (0 new commits since #189/`5bd345b`) — an unchanged-head **RE-CONFIRMATION (11th consecutive on `56ed452`)**, NOT a re-review of frozen content. **Re-confirmed CLEAR; NOT merged** (owner-merge-only standing draft, still `draft:true`). **No new executable rules — version stays v29.** R1-R29 remain current._

### PR Status after this review pass

| PR | Finding | Status |
|----|---------|--------|
| #178 | Clear (re-confirmation). Head `56ed452` unchanged since the 07-10 06:00 review (#180) and re-confirmed at #181-#189; `pull_request_read` → `head.sha=56ed452`, `draft:true`, `merged:false`, master unchanged since `5bd345b` → no new pushed content to re-litigate. Safety invariants re-verified at the tip (MR1, checked not inferred): **R23 CLEAN** (three-dot `git diff --numstat origin/master...origin/feature/ec-screen-deepdive` on all four reviewer-owned docs = EMPTY, zero `-` lines); **R28 PASS** (CLAUDE.md 124 lines, < 200); never-auto-merge holds. **R27 pre-milestone advisory re-confirmed** (owner action, not reviewer): two-dot `git diff --stat origin/master origin/feature/ec-screen-deepdive` shows the branch behind master on `docs/lessons-learned.md -265` + `docs/review-log.md -11` + `STATUS.md` (= the #179-#189 review entries not re-absorbed) — the literal R23 test is the THREE-dot form and it is CLEAN, so NOT an R23 MUST-FIX, but the owner MUST `git merge origin/master` (or `--no-ff`, NOT squash per R27) before the next milestone push/merge or a squash would clobber those review records. **R29** — no new content pushed this cycle, so no stale-runner artifact-format mismatch to flag. **NOT merged** (owner-merge-only standing draft). | ✅ Clear — left open (owner-merge-only) |

### Observations (good patterns to keep)

- **Eleven consecutive re-confirmations on a frozen head is the correct, cheap shape for a standing-draft the reviewer is forbidden to merge.** Runs #180 through this one have found `56ed452` byte-identical every time; each verifies the three safety invariants (never-auto-merge, R23 three-dot clean, R28 size gate) plus the R27 pre-milestone advisory rather than re-scoring already-cleared content. Confirm the invariants, don't re-litigate the diff.
- **R23 (three-dot) vs R27 (two-dot) stay correctly distinguished as the drift grows.** The branch is legitimately behind master on the reviewer-owned docs (two-dot now `-265`/`-11`, grown by one more review entry since #189's `-235`/`-10`) but the authoritative clobber-risk test is the three-dot merge-base diff, which is empty — so this remains an owner pre-milestone-merge action (R27), never an R23 MUST-FIX. Reporting both forms every run keeps the distinction honest.
- **Both open code-hygiene NICE-TO-HAVEs re-checked directly at the tip (MR1), not assumed:** `run_ec_screen_learn.py:104` still carries the dead 7th resolver candidate (`re.sub(...,r'',base)` → always empty, filtered by `if c`), and `gen_checklist.py:33` still emits a U+2014 em-dash to stdout (Python `ord()>127` scan → bytes `0xe2 0x80 0x94`; a Windows `grep -P "[^\x00-\x7F]"` misses it, so the Python scan is authoritative on this platform). Both non-gating carry-overs, unchanged since the head froze.

### Gaps (verified against filesystem / GitHub API)

| Gap | Owner | Priority |
|-----|-------|----------|
| **R27 owner action (pre-milestone):** before the next milestone merge of `feature/ec-screen-deepdive`, `git merge origin/master` (or `--no-ff`, NOT squash) so the #179-#189 reviewer-doc entries are re-absorbed — a squash as-is would clobber the accumulated review records (R23 three-dot is clean today only because the branch does not edit those files; a milestone squash still needs the re-absorb) | Owner | 🟡 Medium |
| Resolver dead 7th candidate `run_ec_screen_learn.py:104` — `re.sub(...,r'',base)` always empty (intended `r'\2'`); carry-over from #170 onward, verified still at tip `56ed452` | Worker | 🟢 Low |
| Carry-over (still open): extend `check_bundle_hygiene.py` ASCII gate to `.claude/skills/**/*.py` + `workstreams/**/scripts/*.py` + `tools/**`; `DeepDiveLearnings/ec-screens/gen_checklist.py:33` em-dash `print` (R18/R20, verified still present via Python ord-scan); `ec-sql-script-builder` demo SQL `REV_TEXT='ECPR-XXXX'` → `'ECPR-DEMO'` (R22); `EXECUTION-PLAN-completion.md` Phase 4 still prescribes squash (R27 — switch to `--no-ff`/rebase); R9 header drift on the bot-generated draft body (harmless for a never-auto-merged draft); ECIS `upload -> RUN NOW` flakiness root cause; Reported Alarms EVENT_LOG clone; WR.0010.02 Well Oil Comp automation | Worker | 🟡 Medium |

### Reviewer process note (isolated worktree; stale main-checkout docs)

- The main checkout (`C:\Projects\ChoongYin_OS`) is the Worker's permanent branch `feature/ec-screen-deepdive` at a stale tip (`a7a0a3d`, ~104 commits behind master), so the session-start mandatory-read `docs/lessons-learned.md`/`docs/review-log.md` there were the STALE **v23** copies; the live **v29** docs (R1-R29) were re-read from the `origin/master` copy in the worktree before any rule/version decision (MR4). ALL review-doc edits were made in an isolated `C:/tmp/wt-review-2026-07-16-0600` worktree off `origin/master`; the Worker's dirty permanent-branch checkout and all sibling `wt-*` worktrees (incl. the runner's `wt-ec-learn`) were never touched. Steps 4b/18's `git checkout master` in the main checkout would disrupt the parallel Worker session and were deliberately NOT run there.

---

## 2026-07-17 06:00 AWST — Automated Review (0 new master commits since #190; 1 open PR #178, STANDING/DRAFT)

_Open PR triggers a full review (R14). #178's head `56ed452` is byte-identical to the commit cleared at #180 through #190 and master has not advanced (0 new commits since #190/`ba27009`) — an unchanged-head **RE-CONFIRMATION (12th consecutive on `56ed452`)**, NOT a re-review of frozen content. **Re-confirmed CLEAR; NOT merged** (owner-merge-only standing draft, still `draft:true`). **No new executable rules — version stays v29.** R1-R29 remain current._

### PR Status after this review pass

| PR | Finding | Status |
|----|---------|--------|
| #178 | Clear (re-confirmation). Head `56ed452` unchanged since the 07-10 06:00 review (#180) and re-confirmed at #181-#190; `list_pull_requests`/`pull_request_read` → the only open PR = #178, `head.sha=56ed452`, `isDraft:true`, `merged:false`, master unchanged since `ba27009` → no new pushed content to re-litigate. Safety invariants re-verified at the tip (MR1, checked not inferred): **R23 CLEAN** (three-dot `git diff --numstat origin/master...origin/feature/ec-screen-deepdive` on all four reviewer-owned docs = EMPTY, zero `-` lines); **R28 PASS** (CLAUDE.md 124 lines, < 200); never-auto-merge holds. **R27 pre-milestone advisory re-confirmed** (owner action, not reviewer): two-dot `git diff --stat origin/master origin/feature/ec-screen-deepdive` shows the branch behind master on `docs/lessons-learned.md -295` + `docs/review-log.md -12` + `STATUS.md` (= the #179-#190 review entries not re-absorbed) — the literal R23 test is the THREE-dot form and it is CLEAN, so NOT an R23 MUST-FIX, but the owner MUST `git merge origin/master` (or `--no-ff`, NOT squash per R27) before the next milestone push/merge or a squash would clobber those review records. **R29** — runner `run_ec_screen_learn.py` re-scanned at the tip via a Python `ord()>127` scan = 0 non-ASCII bytes over 368 lines; no new content pushed this cycle → no stale-runner artifact-format mismatch to flag. **NOT merged** (owner-merge-only standing draft). | ✅ Clear — left open (owner-merge-only) |

### Observations (good patterns to keep)

- **Twelve consecutive re-confirmations on a frozen head is the correct, cheap shape for a standing-draft the reviewer is forbidden to merge.** Runs #180 through this one have found `56ed452` byte-identical every time; each verifies the three safety invariants (never-auto-merge, R23 three-dot clean, R28 size gate) plus the R27 pre-milestone advisory rather than re-scoring already-cleared content. Confirm the invariants, don't re-litigate the diff.
- **R23 (three-dot) vs R27 (two-dot) stay correctly distinguished as the drift grows.** The branch is legitimately behind master on the reviewer-owned docs (two-dot now `-295`/`-12`, grown by one more review entry since #190's `-265`/`-11`) but the authoritative clobber-risk test is the three-dot merge-base diff, which is empty — so this remains an owner pre-milestone-merge action (R27), never an R23 MUST-FIX. Reporting both forms every run keeps the distinction honest.
- **Both open code-hygiene NICE-TO-HAVEs re-checked directly at the tip (MR1), not assumed:** `run_ec_screen_learn.py:104` still carries the dead 7th resolver candidate (`re.sub(...,r'',base)` → always empty, filtered by `if c`), and `gen_checklist.py:33` still emits a U+2014 em-dash to stdout (R18/R20 cp1252 crash risk; markdown `.write_text(...,encoding="utf-8")` lines exempt). Both non-gating carry-overs, unchanged since the head froze.

### Gaps (verified against filesystem / GitHub API)

| Gap | Owner | Priority |
|-----|-------|----------|
| **R27 owner action (pre-milestone):** before the next milestone merge of `feature/ec-screen-deepdive`, `git merge origin/master` (or `--no-ff`, NOT squash) so the #179-#190 reviewer-doc entries are re-absorbed — a squash as-is would clobber the accumulated review records (R23 three-dot is clean today only because the branch does not edit those files; a milestone squash still needs the re-absorb) | Owner | 🟡 Medium |
| Resolver dead 7th candidate `run_ec_screen_learn.py:104` — `re.sub(...,r'',base)` always empty (intended `r'\2'`); carry-over from #170 onward, verified still at tip `56ed452` | Worker | 🟢 Low |
| Carry-over (still open): extend `check_bundle_hygiene.py` ASCII gate to `.claude/skills/**/*.py` + `workstreams/**/scripts/*.py` + `tools/**`; `DeepDiveLearnings/ec-screens/gen_checklist.py:33` em-dash `print` (R18/R20, verified still present); `ec-sql-script-builder` demo SQL `REV_TEXT='ECPR-XXXX'` → `'ECPR-DEMO'` (R22); `EXECUTION-PLAN-completion.md` Phase 4 still prescribes squash (R27 — switch to `--no-ff`/rebase); R9 header drift on the bot-generated draft body (harmless for a never-auto-merged draft); ECIS `upload -> RUN NOW` flakiness root cause; Reported Alarms EVENT_LOG clone; WR.0010.02 Well Oil Comp automation | Worker | 🟡 Medium |

...
| Royalty Objects | Unit - Well Setup (PC, gated) — 6/8 | ✅ Live 3/3 (2026-06-27) | 6th Royalty Objects screen, 2nd PC (after Object List Setup); gated by Unit Agreement nav dd; clone of Object List Setup; view `DV_UNIT_WELL_SETUP` (count-delta on `PERF_INTERVAL_CODE`); physical membership delete; saved-row select via `C0_in` (NEW row calendar vs SAVED text) cost 1 live run then fixed; test pair UNIT_3 (empty) × 108_WB1-1_PF1; no shared-file edits; DB-verified + self-cleaning |

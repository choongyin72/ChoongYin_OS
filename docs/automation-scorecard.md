...
| N1 | Composition edit (per-component) | STRM-OIL-COMP (PO.0019) | ✅ Live 3/3 | Oil uses **WT_PCT** (`C2_in`); `C1_in` (MOL_PCT) is empty/read-only for oil. Scope = "P1 Facility Allocation". ANALYSIS_TYPE='OILCOMP'. Same `component_value_should_be` DbVerify; Ethane(C2) guard; TC03 reload-before-revert. TARGET_STREAM=P1 Alloc S001 M OIL. | 18 Jun |
| N1 | Composition edit (per-component) | WELL-GAS-COMP (WR.0010.01) | ✅ Live 3/3 | Well gas uses MOL_PCT like stream gas. **Extra step**: after GO, an analysis header grid loads; must click header row (`Select Analysis Row` via JS on `C0_in`) before component grid appears. TC03 = reload + RE-SELECT + re-cache. WELL_SOURCE=WELL_VERSION. 9-field nav (PU/Area/Facility/Well scope). Ethane(C2) guard; R12 N/A. | 18 Jun |

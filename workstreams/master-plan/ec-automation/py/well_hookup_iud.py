"""Well Hookup - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/well_hookup_iud.py
"""
import os
import sys
...
VIEW          = 'ov_well_hookup'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_WH_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Well Hookup 001'
NAME_UPD      = 'AUTOTEST Well Hookup 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'well_hookup' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("wh_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Well Hookup Code', "value": CODE,       "kind": "text"},
                {"label": 'Well Hookup Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "Op Production Unit", "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Well Hookup Name', "value": NAME_UPD, "kind": "text"}]

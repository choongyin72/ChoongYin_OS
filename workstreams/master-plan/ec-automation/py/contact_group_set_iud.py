"""Maintain Contact Group Set - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.

OV-GM (grid manageObject:form:T_data) = navigator-GATED (cascade + GO before the grid loads). Built on the
gated-navigator capability (apply_ovgm_navigator). Fields by label. No Op Production Unit field or extra dropdowns on this screen (confirmed live).
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/contact_group_set_iud.py
"""
import os
import sys
...
SCREEN        = 'Maintain Contact Group Set'
GRID_DATA_ID  = "manageObject:form:T_data"
VIEW          = 'ov_contact_group_set'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CGS_001")
START_DATE    = "2020-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Maintain Contact Group Set 001'
NAME_UPD      = 'AUTOTEST Maintain Contact Group Set 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'contact_group_set' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("cgs_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Contact Group Set Code', "value": CODE,       "kind": "text"},
                {"label": 'Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Functional Area', "value": pu, "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Name', "value": NAME_UPD, "kind": "text"}]

"""Task Process - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Task List, CO.0191). Mandatory: Task Process Code/Task Process Name/Start date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/task_process.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/task_process_iud.py
"""
import os
import sys
...
VIEW          = "ov_task_process"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_TP_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Task Process 001"
NAME_UPD      = "AUTOTEST Task Process 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Task Process Code",  "value": CODE,       "kind": "text"},
    {"label": "Task Process Name",  "value": NAME,       "kind": "text"},
    {"label": "Start date",  "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Task Process Name",  "value": NAME_UPD,   "kind": "text"},
]
...
EVID = _ROOT / "tmp" / "task_process" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"task_process_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_task_process"

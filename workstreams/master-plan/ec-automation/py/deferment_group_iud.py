"""Deferment Group - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Facility Objects, CO.0149). Mandatory: Deferment Group Code/Deferment Group Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/deferment_group.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/deferment_group_iud.py
"""
import os
import sys
...
VIEW          = "ov_deferment_group"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_DG_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Deferment Group 001"
NAME_UPD      = "AUTOTEST Deferment Group 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Deferment Group Code",  "value": CODE,       "kind": "text"},
    {"label": "Deferment Group Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Deferment Group Name",  "value": NAME_UPD,   "kind": "text"},
]
...
EVID = _ROOT / "tmp" / "deferment_group" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"deferment_group_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_deferment_group"

"""Inventory Area - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Inventory Objects, CD.0115). Mandatory: Inventory Area Code/Inventory Area Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/inventory_area.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/inventory_area_iud.py
"""
import os
import sys
...
VIEW          = "ov_inventory_area"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_INVA_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Inventory Area 001"
NAME_UPD      = "AUTOTEST Inventory Area 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Inventory Area Code",  "value": CODE,       "kind": "text"},
    {"label": "Inventory Area Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Inventory Area Name",  "value": NAME_UPD,   "kind": "text"},
]
...
EVID = _ROOT / "tmp" / "inventory_area" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"inventory_area_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_inventory_area"

"""Test Device - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/test_device_iud.py
"""
import os
import sys
...
VIEW          = 'ov_test_device'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_TD_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Test Device 001'
NAME_UPD      = 'AUTOTEST Test Device 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'test_device' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("td_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Test Device Code', "value": CODE,       "kind": "text"},
                {"label": 'Test Device Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "Op Production Unit", "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Test Device Name', "value": NAME_UPD, "kind": "text"}]

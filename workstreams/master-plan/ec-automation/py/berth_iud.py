"""Berth - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Transport Objects, CO.2012). Mandatory: Berth Code/Name/Start Date (Port Name/Business Unit/
Capacity/Op dropdowns all optional -> skipped). Update = Name. Selector map: ec-ui-knowledge/screens/berth.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/berth_iud.py
"""
import os
import sys
...
VIEW          = "ov_berth"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_BERTH_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Berth 001"
NAME_UPD      = "AUTOTEST Berth 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Berth Code",  "value": CODE,       "kind": "text"},
    {"label": "Berth Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Berth Name",  "value": NAME_UPD,   "kind": "text"},
]
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"berth_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_berth"

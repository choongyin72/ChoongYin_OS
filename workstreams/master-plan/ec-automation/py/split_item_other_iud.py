"""Split Item Other - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Revenue Split Keys, CD.0017). Mandatory: Split Item Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/split_item_other.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/split_item_other_iud.py
"""
import os
import sys
...
VIEW          = "ov_split_item_other"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_SIO_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Split Item Other 001"
NAME_UPD      = "AUTOTEST Split Item Other 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Split Item Code",  "value": CODE,       "kind": "text"},
...
EVID = _ROOT / "tmp" / "split_item_other" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"split_item_other_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_split_item_other"

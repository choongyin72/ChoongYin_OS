"""Price Object - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.

OV-GM (grid manageObject:form:T_data) = navigator-GATED (cascade + GO before the grid loads). Built on the
gated-navigator capability (apply_ovgm_navigator). Fields by label. Extra dropdowns set first-available (probe per screen).
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/price_object_iud.py
"""
import os
import sys
...
VIEW          = 'ov_price_object'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_PO_001")
START_DATE    = "2020-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Price Object 001'
NAME_UPD      = 'AUTOTEST Price Object 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'price_object' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("po_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            pu = ec.apply_ovgm_navigator(page, levels=1)
            ...
            insert_fields = [
                {"label": 'Price Object Code', "value": CODE,       "kind": "text"},
                {"label": 'Price Object Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Product Name', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Price Concept', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'UOM', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Price Rounding Rule', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Business Unit', "value": pu, "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Price Object Name', "value": NAME_UPD, "kind": "text"}]

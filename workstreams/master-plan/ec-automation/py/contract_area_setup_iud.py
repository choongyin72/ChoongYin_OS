"""Contract Area Setup - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.

CUSTOM-URL OV (grid nav:form:T_data): NO navigator, NO GO button - the engine's click_go falls back
to the toolbar Refresh icon. Extra mandatory over plain OV: Contract Area Name + Contract Name
reference dropdowns (first-available). START_DATE=2020-01-01 (NOT 2000-01-01): ref dropdowns only
offer objects effective at the form Start Date - DB-verified 28 contract areas + 98 contracts
effective at 2020-01-01 on this sandbox.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/contract_area_setup_iud.py
"""
import os
import sys
...
VIEW          = "ov_contract_area_setup"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CAS_001")
START_DATE    = "2020-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Contract Area Setup 001"
NAME_UPD      = "AUTOTEST Contract Area Setup 001 UPDATED"
...
EVID = _ROOT / "tmp" / "contract_area_setup" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"cas_{label}.png"))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": "Contract Area Setup Code", "value": CODE,       "kind": "text"},
                {"label": "Contract Area Setup Name", "value": NAME,       "kind": "text"},
                {"label": "Start Date",               "value": START_DATE, "kind": "date"},
                {"label": "Contract Area Name",       "value": "__FIRST__", "kind": "dropdown"},
                {"label": "Contract Name",            "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": "Contract Area Setup Name", "value": NAME_UPD, "kind": "text"}]

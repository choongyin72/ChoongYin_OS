"""Report Area - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

Per-screen template instance. Simplest OV: Code/Name/Start Date only (no Description, no dropdowns).
Selector map: ec-ui-knowledge/screens/report_area.md ; pattern: ec-ui-knowledge/EC_OBJECT_CONFIG_IUD.md
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/report_area_iud.py
"""
import os
import sys
...
VIEW          = "ov_report_area"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_RPTA_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Report Area 001"
NAME_UPD      = "AUTOTEST Report Area 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Report Area Code", "value": CODE,       "kind": "text"},
    {"label": "Report Area Name", "value": NAME,       "kind": "text"},
    {"label": "Start date",       "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Report Area Name", "value": NAME_UPD,   "kind": "text"},
]
...
EVID = _ROOT / "tmp" / "report_area" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"rpta_{label}.png"))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            ec.click_go(page)  # grid needs GO to populate

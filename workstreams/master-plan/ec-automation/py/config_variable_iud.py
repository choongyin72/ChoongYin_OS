"""Config Variable - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Calculation Objects, IN.0031). Mandatory: Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/config_variable.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/config_variable_iud.py
"""
import os
import sys
...
VIEW          = "ov_config_variable"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CV_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Config Variable 001"
NAME_UPD      = "AUTOTEST Config Variable 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Code",  "value": CODE,       "kind": "text"},
    {"label": "Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
    {"label": "Calculation Context", "value": "Cargo Load/Unload", "kind": "dropdown"},
]
UPDATE_FIELDS = [
...
EVID = _ROOT / "tmp" / "config_variable" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"config_variable_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_config_variable"

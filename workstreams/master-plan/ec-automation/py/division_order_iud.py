"""Division Order - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/division_order_iud.py
"""
import os
import sys
...
VIEW          = 'ov_division_order'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_DO_001")
START_DATE    = "2003-01-01"
END_DATE      = START_DATE
INSERT_END_DATE = "2099-12-31"   # End Date is functionally mandatory on Save despite not being
                                  # flagged yellow on live scan (same as Royalty Contract/Contract);
                                  # NOT the same as END_DATE above, which is the delete gesture's value
NAME          = 'AUTOTEST Division Order 001'
NAME_UPD      = 'AUTOTEST Division Order 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'division_order' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("do_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            ec.select_dropdown(page, "nav:form:G:1:R:1:C:0:dd_input", 'Royalty USA')
            ec.click_go(page)
            pu = 'Royalty USA'
            ...
            insert_fields = [
                {"label": 'Division Order Code', "value": CODE,       "kind": "text"},
                {"label": 'Division Order Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "End Date", "value": INSERT_END_DATE, "kind": "date"},
                {"label": 'Contract Template', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Contract Area', "value": 'Louisiana North', "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Division Order Name', "value": NAME_UPD, "kind": "text"}]

"""Canal - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Transport Objects, CO.2069). Mandatory: Canal Code/Name/Start Date (Time Zone dropdown optional
-> skipped). Update = Name. Selector map: ec-ui-knowledge/screens/canal.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/canal_iud.py
"""
import os
import sys
...
VIEW          = "ov_canal"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CANAL_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Canal 001"
NAME_UPD      = "AUTOTEST Canal 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Canal Code",  "value": CODE,       "kind": "text"},
    {"label": "Canal Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Canal Name",  "value": NAME_UPD,   "kind": "text"},
]
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"canal_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_canal"

"""Test Separator - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/test_separator_iud.py
"""
import os
import sys
...
VIEW          = 'ov_testseparator'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_TSEP_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Test Separator 001'
NAME_UPD      = 'AUTOTEST Test Separator 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'test_separator' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("tsep_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Test Separator Code', "value": CODE,       "kind": "text"},
                {"label": 'Test Separator Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "Op Production Unit", "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Test Separator Name', "value": NAME_UPD, "kind": "text"}]

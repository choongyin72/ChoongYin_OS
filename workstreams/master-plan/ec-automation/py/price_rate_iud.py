"""Price Rate - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/price_rate_iud.py
"""
import os
import sys
...
VIEW          = 'ov_price_rate'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_PRT_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Price Rate 001'
NAME_UPD      = 'AUTOTEST Price Rate 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'price_rate' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("prt_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            ec.select_dropdown(page, "nav:form:G:0:R:1:C:1:dd_input", 'SS2 BU')
            ec.click_go(page)
            pu = 'SS2 BU'
            ...
            insert_fields = [
                {"label": 'Price Rate Code', "value": CODE,       "kind": "text"},
                {"label": 'Price Rate Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Frequency', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Business Unit', "value": pu, "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Price Rate Name', "value": NAME_UPD, "kind": "text"}]

"""Storage - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/storage_iud.py
"""
import os
import sys
...
VIEW          = 'ov_storage'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_STG_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Storage 001'
NAME_UPD      = 'AUTOTEST Storage 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'storage' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("stg_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Storage Code', "value": CODE,       "kind": "text"},
                {"label": 'Storage Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Storage Type', "value": "__FIRST__", "kind": "dropdown"},
                {"label": 'Product Name', "value": "__FIRST__", "kind": "dropdown"},
                {"label": "Op Production Unit", "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Storage Name', "value": NAME_UPD, "kind": "text"}]

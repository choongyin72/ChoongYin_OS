"""Operator Route - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/operator_route_iud.py
"""
import os
import sys
...
VIEW          = 'ov_operator_route'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_OR_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Operator Route 001'
NAME_UPD      = 'AUTOTEST Operator Route 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'operator_route' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("or_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Operator Route Code', "value": CODE,       "kind": "text"},
                {"label": 'Operator Route Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
            ]
            update_fields = [{"label": 'Operator Route Name', "value": NAME_UPD, "kind": "text"}]

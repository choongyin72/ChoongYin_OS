"""Pilot Boat - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/pilot_boat_iud.py
"""
import os
import sys
...
VIEW          = 'ov_pilot_boat'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_PB_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Pilot Boat 001'
NAME_UPD      = 'AUTOTEST Pilot Boat 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'pilot_boat' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("pb_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Pilot Boat Code', "value": CODE,       "kind": "text"},
                {"label": 'Pilot Boat Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "Op Production Unit", "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Pilot Boat Name', "value": NAME_UPD, "kind": "text"}]

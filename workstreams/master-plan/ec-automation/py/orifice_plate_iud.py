"""Orifice Plate - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Stream Objects, CO.0089). Mandatory: Orifice Code/Orifice Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/orifice_plate.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/orifice_plate_iud.py
"""
import os
import sys
...
VIEW          = "ov_orifice_plate"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_OP_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Orifice Plate 001"
NAME_UPD      = "AUTOTEST Orifice Plate 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Orifice Code",  "value": CODE,       "kind": "text"},
    {"label": "Orifice Name",  "value": NAME,       "kind": "text"},
...
EVID = _ROOT / "tmp" / "orifice_plate" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"orifice_plate_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_orifice_plate"

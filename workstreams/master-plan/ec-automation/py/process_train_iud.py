"""Process Train - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Facility Objects, CO.0120). Mandatory: Process Train Code/Process Train Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/process_train.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/process_train_iud.py
"""
import os
import sys
...
VIEW          = "ov_process_train"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_PT_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Process Train 001"
NAME_UPD      = "AUTOTEST Process Train 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Process Train Code",  "value": CODE,       "kind": "text"},
    {"label": "Process Train Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
    {"label": "Production Facility Class 1", "value": "__FIRST__", "kind": "dropdown"},
]
UPDATE_FIELDS = [
    {"label": "Process Train Name",  "value": NAME_UPD,   "kind": "text"},
]
...
EVID = _ROOT / "tmp" / "process_train" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"process_train_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_process_train"

"""Loading Arm - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/loading_arm_iud.py
"""
import os
import sys
...
END_DATE      = START_DATE
NAME          = 'AUTOTEST Loading Arm 001'
NAME_UPD      = 'AUTOTEST Loading Arm 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'loading_arm' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("la_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Loading Arm Code', "value": CODE,       "kind": "text"},
                {"label": 'Loading Arm Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "Op Production Unit", "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Loading Arm Name', "value": NAME_UPD, "kind": "text"}]

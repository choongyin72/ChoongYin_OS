"""Action Trigger - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py UNCHANGED.

CUSTOM-URL OV (grid nav:form:T_data, VERSIONED): NO navigator, NO GO button - confirmed live
(tmp/action_trgger_screen.log: navigator fields=[], go=[]). BF CO.0193, view OV_CONTROL_POINT
(class CONTROL_POINT, TIME_SCOPE_CODE=VERSIONED -> End=Start delete), both cross-checked against the
DB/treeview directly, not the (previously-wrong-3-times) doc alone. 5 mandatory insert fields: Code,
Name, Start Date, Action Trigger Type (dd, 3 options probed non-empty), Trigger Type (dd, 2 options
probed non-empty) - neither is scope-dependent (no navigator to disagree with), so __FIRST__ is safe.

OPT-IN MANDATORY-FIELD GATE: this driver is the FIRST to adopt the standalone
mandatory_field_gate.py (owner-directed, built/tested 2026-08-01). It is a PROACTIVE pre-flight check -
before Save is ever clicked, scan for mandatory-and-empty fields and fail with the exact list, instead
of relying only on EC's own post-Save error banner (ec_error()). Implemented as LOCAL wrapper functions
(insert_with_gate/update_with_gate) that replicate insertObjectRecord/updateObjectRecord's exact steps
using the SAME already-exposed primitives (_open_new_object, _resolve_field, fill_field, save,
ec_error, click_go) - ec_object_iud.py itself is NOT modified, so every other shipped screen (18+) is
completely unaffected by this opt-in.

Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/action_trigger_iud.py
"""
import os
import sys
...
GRID_DATA_ID  = "nav:form:T_data"   # custom-URL OV: no manage_object_nav prefix, no GO
VIEW          = "ov_control_point"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_AT_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Action Trigger 001"
NAME_UPD      = "AUTOTEST Action Trigger 001 UPDATED"
...
EVID = _ROOT / "tmp" / "action_trigger" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"at_{label}.png"))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            # custom-URL OV: no navigator cascade, grid loads directly

            insert_fields = [
                {"label": "Action Trigger Code", "value": CODE,       "kind": "text"},
                {"label": "Action Trigger Name", "value": NAME,       "kind": "text"},
                {"label": "Start date",          "value": START_DATE, "kind": "date"},
                {"label": "Action Trigger Type", "value": "__FIRST__", "kind": "dropdown"},
                {"label": "Trigger Type",         "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": "Action Trigger Name", "value": NAME_UPD, "kind": "text"}]

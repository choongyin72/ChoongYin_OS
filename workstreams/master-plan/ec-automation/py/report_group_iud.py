"""Report Group - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.

PLAIN OV (Bank family, grid report_group_table:form:T_data): navigator is date-only - no mandatory
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/report_group_iud.py
"""
import os
import sys
...
SCREEN        = 'Report Group'
GRID_DATA_ID  = "report_group_table:form:T_data"
VIEW          = 'ov_report_group'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_RG001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Report Group 001'
NAME_UPD      = 'AUTOTEST Report Group 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'report_group' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("RG_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Reporting Group Code', "value": CODE,       "kind": "text"},
                {"label": 'Reporting Group Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Description', "value": 'AUTOTEST Report Group description', "kind": "text"},
                {"label": 'Business Area', "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Reporting Group Name', "value": NAME_UPD, "kind": "text"}]

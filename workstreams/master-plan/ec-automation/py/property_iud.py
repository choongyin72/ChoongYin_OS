"""Property - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/property_iud.py
"""
import os
import sys
...
VIEW          = 'ov_property'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_PROP001")
START_DATE    = "2003-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Property 001'
NAME_UPD      = 'AUTOTEST Property 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'property' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("PROP_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Property Code', "value": CODE,       "kind": "text"},
                {"label": 'Property Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Business Unit Name', "value": 'Royalty Canada', "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Property Name', "value": NAME_UPD, "kind": "text"}]

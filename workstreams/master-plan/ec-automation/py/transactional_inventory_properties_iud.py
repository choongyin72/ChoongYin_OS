"""Transactional Inventory Properties - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Configuration, IN.0023). Mandatory: Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/transactional_inventory_properties.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/transactional_inventory_properties_iud.py
"""
import os
import sys
...
VIEW          = "ov_trans_inventory"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_TIP_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Transactional Inventory Properties 001"
NAME_UPD      = "AUTOTEST Transactional Inventory Properties 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Code",  "value": CODE,       "kind": "text"},
    {"label": "Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
    {"label": "Sequence Number", "value": "1", "kind": "text"},
]
UPDATE_FIELDS = [
...
EVID = _ROOT / "tmp" / "transactional_inventory_properties" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"transactional_inventory_properties_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_trans_inventory"

"""Blend - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Hydrocarbon Objects, CO.0219). Mandatory: Blend Code/Blend Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/blend.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/blend_iud.py
"""
import os
import sys
...
VIEW          = "ov_blend"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_BLEND_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Blend 001"
NAME_UPD      = "AUTOTEST Blend 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Blend Code",  "value": CODE,       "kind": "text"},
    {"label": "Blend Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Blend Name",  "value": NAME_UPD,   "kind": "text"},
]
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"blend_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_blend"

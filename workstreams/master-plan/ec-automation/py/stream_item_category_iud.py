"""Stream Item Category - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Stream Objects, CD.0016). Mandatory: Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/stream_item_category.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/stream_item_category_iud.py
"""
import os
import sys
...
VIEW          = "ov_stream_item_category"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_SIC_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Stream Item Category 001"
NAME_UPD      = "AUTOTEST Stream Item Category 001 UPDATED"
...
EVID = _ROOT / "tmp" / "stream_item_category" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"stream_item_category_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_stream_item_category"

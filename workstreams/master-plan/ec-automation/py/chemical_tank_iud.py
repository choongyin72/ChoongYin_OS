"""Chemical Tank - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV-GM (grid manageObject:form:T_data) = navigator-GATED (Production Unit -> Area -> Facility Class 1
cascade + GO before the grid loads). Sibling of Node. Extra mandatory over the plain OV: Measure unit
(dropdown). Op Production Unit set first-available for grid visibility (probe per screen - see Node).
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/chemical_tank_iud.py
"""
import os
import sys
...
GRID_DATA_ID  = "manageObject:form:T_data"   # OV-GM
VIEW          = "ov_chem_tank"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CT_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Chemical Tank 001"
NAME_UPD      = "AUTOTEST Chemical Tank 001 UPDATED"
...
EVID = _ROOT / "tmp" / "chemical_tank" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"ct_{label}.png"))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": "Chemical Tank Code", "value": CODE,       "kind": "text"},
                {"label": "Chemical Tank Name", "value": NAME,       "kind": "text"},
                {"label": "Start Date",         "value": START_DATE, "kind": "date"},
                {"label": "Measure unit",       "value": "__FIRST__", "kind": "dropdown"},
                {"label": "Op Production Unit",  "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": "Chemical Tank Name", "value": NAME_UPD, "kind": "text"}]

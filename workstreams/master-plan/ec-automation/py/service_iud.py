...
VIEW          = 'ov_service'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_SV001")
START_DATE    = "2011-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Service 001'
NAME_UPD      = 'AUTOTEST Service 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'service' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("SV_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            ec.select_dropdown(page, "nav:form:G:0:R:1:C:1:dd_input", 'TS3 BU1')
            ec.click_go(page)
            pu = 'TS3 BU1'
            ...
            insert_fields = [
                {"label": 'Service Code', "value": CODE,       "kind": "text"},
                {"label": 'Service Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Service Template', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Service Type', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Service Status', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Contract', "value": 'TS3 GTA Shipper A', "kind": "dropdown"},
                {"label": 'Transport System', "value": 'TS3 Transport System', "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Service Name', "value": NAME_UPD, "kind": "text"}]

"""External Location - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/external_location_iud.py
"""
import os
import sys
...
CODE          = os.environ.get("EC_CODE", "AUTOTEST_EL001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST External Location 001'
NAME_UPD      = 'AUTOTEST External Location 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'external_location' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("EL_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            ec.click_go(page)   # navigator fields are optional FILTERS - GO alone loads
            ec.wait_ajax(page)
            ...
            # nav_mode=go_only: pu is legitimately None (optional filters, no scope)

            insert_fields = [
                {"label": 'External Location Code', "value": CODE,       "kind": "text"},
                {"label": 'External Location Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
            ]
            update_fields = [{"label": 'External Location Name', "value": NAME_UPD, "kind": "text"}]

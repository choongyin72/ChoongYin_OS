"""Choke - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Well and Reservoir Objects, CO.0185). Mandatory: Choke Code/Name/Start Date (optional Choke Type dd skipped).
Update covers Name + Comments. Selector map: ec-ui-knowledge/screens/choke.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/choke_iud.py
"""
import os
import sys
...
VIEW          = "ov_choke"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CHK_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Choke 001"
NAME_UPD      = "AUTOTEST Choke 001 UPDATED"
CMT           = "AUTOTEST cmt"
CMT_UPD       = "AUTOTEST cmt UPDATED"

INSERT_FIELDS = [
    {"label": "Choke Code", "value": CODE,       "kind": "text"},
    {"label": "Choke Name", "value": NAME,       "kind": "text"},
    {"label": "Start Date", "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Choke Name", "value": NAME_UPD, "kind": "text"},
    {"label": "Comments",   "value": CMT_UPD,  "kind": "text"},
]
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"choke_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_choke"
            ...
            def _v_upd():
                ...
                ok_c, ac = db.field_equals(VIEW, CODE, "COMMENTS", CMT_UPD)
                assert ok_n, f"DB NAME={an!r} != {NAME_UPD!r}"
                assert ok_c, f"DB COMMENTS={ac!r} != {CMT_UPD!r}"
            step(page, "update_db", _v_upd)
            print("  UPDATE verified (DB NAME + COMMENTS)")

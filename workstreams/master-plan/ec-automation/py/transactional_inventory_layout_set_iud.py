"""Transactional Inventory Layout Set - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Configuration, IN.0033). Mandatory: Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/transactional_inventory_layout_set.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/transactional_inventory_layout_set_iud.py
"""
import os
import sys
...
VIEW          = "ov_trans_inv_tmpl_set"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_TILS_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Trans Inv Layout Set 001"
NAME_UPD      = "AUTOTEST Trans Inv Layout Set 001 UPDATED"
...
EVID = _ROOT / "tmp" / "transactional_inventory_layout_set" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"transactional_inventory_layout_set_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_trans_inv_tmpl_set"

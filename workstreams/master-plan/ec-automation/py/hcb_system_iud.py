"""HCB System - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Revenue Lists, CD.0097). Mandatory: Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/hcb_system.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/hcb_system_iud.py
"""
import os
import sys
...
VIEW          = "ov_balance"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_HCB_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST HCB System 001"
NAME_UPD      = "AUTOTEST HCB System 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Code",  "value": CODE,       "kind": "text"},
    {"label": "Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
    {"label": "HCB Category", "value": "Operating area system", "kind": "dropdown"},
]
UPDATE_FIELDS = [
...
EVID = _ROOT / "tmp" / "hcb_system" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"hcb_system_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_balance"

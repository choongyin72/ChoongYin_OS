"""Document Sequence - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Revenue Document Objects, CD.0109). Mandatory: Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/document_sequence.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/document_sequence_iud.py
"""
import os
import sys
...
VIEW          = "ov_doc_sequence"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_DS_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Doc Sequence 001"
NAME_UPD      = "AUTOTEST Doc Sequence 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Code",  "value": CODE,       "kind": "text"},
    {"label": "Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
    {"label": "Starting Point", "value": "1", "kind": "text"},
]
UPDATE_FIELDS = [
...
EVID = _ROOT / "tmp" / "document_sequence" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"document_sequence_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_doc_sequence"

"""Price Index - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/price_index_iud.py
"""
import os
import sys
...
END_DATE      = START_DATE
NAME          = 'AUTOTEST Price Index 001'
NAME_UPD      = 'AUTOTEST Price Index 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'price_index' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("pi_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            ec.select_dropdown(page, "nav:form:G:0:R:1:C:1:dd_input", 'Royalty Canada')
            ec.click_go(page)
            pu = 'Royalty Canada'
            ...
            insert_fields = [
                {"label": 'Price Index Code', "value": CODE,       "kind": "text"},
                {"label": 'Price Index Name', "value": NAME,       "kind": "text"},
            ...
            update_fields = [{"label": 'Price Index Name', "value": NAME_UPD, "kind": "text"}]

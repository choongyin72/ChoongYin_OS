"""Contract Capacity - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/contract_capacity_iud.py
"""
import os
import sys
...
VIEW          = 'ov_contract_capacity'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CC001")
START_DATE    = "2020-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Contract Capacity 001'
NAME_UPD      = 'AUTOTEST Contract Capacity 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'contract_capacity' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("CC_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Contract Capacity Code', "value": CODE,       "kind": "text"},
                {"label": 'Contract Capacity Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Contract Name', "value": 'TS5 Shipper B Firm', "kind": "dropdown"},
                {"label": 'Location Name', "value": 'TS5 Domestic Gas Storage', "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Contract Capacity Name', "value": NAME_UPD, "kind": "text"}]

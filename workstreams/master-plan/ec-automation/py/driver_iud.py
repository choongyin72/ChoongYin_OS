"""Driver - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/driver_iud.py
"""
import os
import sys
...
SCREEN        = 'Driver'
GRID_DATA_ID  = "driver_object:form:T_data"
VIEW          = 'ov_driver'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_DR_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Driver 001'
NAME_UPD      = 'AUTOTEST Driver 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'driver' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("dr_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Driver Code', "value": CODE,       "kind": "text"},
                {"label": 'Driver Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Driver Licence No', "value": 'AUTOTEST-LIC-001', "kind": "text"},
                {"label": 'Gender of driver', "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Driver Name', "value": NAME_UPD, "kind": "text"}]

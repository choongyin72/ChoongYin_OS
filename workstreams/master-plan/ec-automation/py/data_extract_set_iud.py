"""Data Extract Set - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Data Mapping Objects, SP.0049). Mandatory: Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/data_extract_set.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/data_extract_set_iud.py
"""
import os
import sys
...
VIEW          = "ov_summary_set"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_DXT_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Data Extract Set 001"
NAME_UPD      = "AUTOTEST Data Extract Set 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Code",  "value": CODE,       "kind": "text"},
    {"label": "Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
    {"label": "Owner Class", "value": "All", "kind": "dropdown"},
]
UPDATE_FIELDS = [
...
EVID = _ROOT / "tmp" / "data_extract_set" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"data_extract_set_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_summary_set"

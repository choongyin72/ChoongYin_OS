"""Pilot - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/pilot_iud.py
"""
import os
import sys
...
VIEW          = 'ov_pilot'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_PL_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Pilot 001'
NAME_UPD      = 'AUTOTEST Pilot 001' + " UPDATED"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("pl_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Pilot Code', "value": CODE,       "kind": "text"},
                {"label": 'Pilot Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "Op Production Unit", "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Pilot Name', "value": NAME_UPD, "kind": "text"}]

"""Input List - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Revenue Lists, CD.0035). Mandatory: Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/input_list.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/input_list_iud.py
"""
import os
import sys
...
VIEW          = "ov_stream_item_collection"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_IL_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Input List 001"
NAME_UPD      = "AUTOTEST Input List 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Code",  "value": CODE,       "kind": "text"},
    {"label": "Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
    {"label": "List Category", "value": "INPUT", "kind": "dropdown"},
]
UPDATE_FIELDS = [
...
EVID = _ROOT / "tmp" / "input_list" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"input_list_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_stream_item_collection"

"""Reservoir Block - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Well and Reservoir Objects, CO.0133). Mandatory: Reservoir Block Code/Reservoir Block Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/reservoir_block.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/reservoir_block_iud.py
"""
import os
import sys
...
VIEW          = "ov_resv_block"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_RESVB_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Reservoir Block 001"
NAME_UPD      = "AUTOTEST Reservoir Block 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Reservoir Block Code",  "value": CODE,       "kind": "text"},
    {"label": "Reservoir Block Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Reservoir Block Name",  "value": NAME_UPD,   "kind": "text"},
]
...
EVID = _ROOT / "tmp" / "reservoir_block" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"reservoir_block_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_resv_block"

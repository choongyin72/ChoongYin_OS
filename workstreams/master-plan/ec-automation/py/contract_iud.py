"""Contract - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/contract_iud.py
"""
import os
import sys
...
END_DATE      = START_DATE
INSERT_END_DATE = "2099-12-31"   # End Date is MANDATORY on this screen's form (unusual);
                                  # distinct from END_DATE (used for the End=Start delete)
NAME          = 'AUTOTEST Contract 001'
NAME_UPD      = 'AUTOTEST Contract 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'contract' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("ct_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            ec.select_dropdown(page, "nav:form:G:0:R:1:C:1:dd_input", 'TS5 BU')
            ec.click_go(page)
            pu = 'TS5 BU'
            ...
            insert_fields = [
                {"label": 'Contract Code', "value": CODE,       "kind": "text"},
                {"label": 'Contract Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "End Date", "value": INSERT_END_DATE, "kind": "date"},
                {"label": "Contract Year Start", "value": "1", "kind": "text"},
                {"label": 'Contract Template', "value": '__FIRST__', "kind": "dropdown"},
                {"label": 'Contract Area', "value": 'TS5 Contract Area', "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Contract Name', "value": NAME_UPD, "kind": "text"}]

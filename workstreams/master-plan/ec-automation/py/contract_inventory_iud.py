"""Contract Inventory - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/contract_inventory_iud.py
"""
import os
import sys
...
VIEW          = 'ov_contract_inventory'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CI_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Contract Inventory 001'
NAME_UPD      = 'AUTOTEST Contract Inventory 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'contract_inventory' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("ci_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            ec.select_dropdown(page, "nav:form:G:0:R:1:C:2:dd_input", 'TS5 Contract Area')
            ec.click_go(page)
            pu = 'TS5 BU'
            ...
            insert_fields = [
                {"label": 'Contract Inventory Code', "value": CODE,       "kind": "text"},
                {"label": 'Contract Inventory Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": 'Contract name', "value": 'TS5 Shipper C', "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Contract Inventory Name', "value": NAME_UPD, "kind": "text"}]

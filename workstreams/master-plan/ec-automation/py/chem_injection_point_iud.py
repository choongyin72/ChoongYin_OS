"""Chemical Injection Point - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.

OV-GM (grid manageObject:form:T_data) = navigator-GATED (Production Unit -> Area -> Facility Class 1
cascade + GO). Sibling of Node/Chemical Tank. Mandatory = Code/Name/Start Date; Op Production Unit set
first-available for grid visibility (probe per screen).
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/chem_injection_point_iud.py
"""
import os
import sys
...
VIEW          = "ov_chem_inj_point"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CIP_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Chem Inj Point 001"
NAME_UPD      = "AUTOTEST Chem Inj Point 001 UPDATED"
...
EVID = _ROOT / "tmp" / "chem_injection_point" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"cip_{label}.png"))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": "Chem Inj Point Code", "value": CODE,       "kind": "text"},
                {"label": "Chem Inj Point Name", "value": NAME,       "kind": "text"},
                {"label": "Start Date",          "value": START_DATE, "kind": "date"},
                {"label": "Op Production Unit",  "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": "Chem Inj Point Name", "value": NAME_UPD, "kind": "text"}]

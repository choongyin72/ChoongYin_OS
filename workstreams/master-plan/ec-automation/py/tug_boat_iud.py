"""Tug Boat - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/tug_boat_iud.py
"""
import os
import sys
...
VIEW          = 'ov_tug_boat'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_TB_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Tug Boat 001'
NAME_UPD      = 'AUTOTEST Tug Boat 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'tug_boat' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("tb_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            insert_fields = [
                {"label": 'Tug Boat Code', "value": CODE,       "kind": "text"},
                {"label": 'Tug Boat Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
                {"label": "Op Production Unit", "value": "__FIRST__", "kind": "dropdown"},
            ]
            update_fields = [{"label": 'Tug Boat Name', "value": NAME_UPD, "kind": "text"}]

"""Collection Point - IUD driver (thin). Reuses shared engine ec_object_iud.py + DbVerify.py.
...
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/collection_point_iud.py
"""
import os
import sys
...
VIEW          = 'ov_collection_point'
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CP001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = 'AUTOTEST Collection Point 001'
NAME_UPD      = 'AUTOTEST Collection Point 001' + " UPDATED"
...
EVID = _ROOT / "tmp" / 'collection_point' / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / ("CP_" + label + ".png")))
    except Exception:
        pass
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            shot(page, "01_loaded")
            ec.select_dropdown(page, "nav:form:G:0:R:1:C:1:dd_input", 'P3 Production Unit')
            ec.select_dropdown(page, "nav:form:G:0:R:1:C:2:dd_input", 'P3 Area')
            ec.select_dropdown(page, "nav:form:G:0:R:1:C:3:dd_input", 'Oper Route 1')
            ec.click_go(page)
            ...
            insert_fields = [
                {"label": 'Collection Point Code', "value": CODE,       "kind": "text"},
                {"label": 'Collection Point Name', "value": NAME,       "kind": "text"},
                {"label": "Start Date", "value": START_DATE, "kind": "date"},
            ]
            update_fields = [{"label": 'Collection Point Name', "value": NAME_UPD, "kind": "text"}]

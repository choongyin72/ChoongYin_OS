"""Revenue Stream Category - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Stream Objects, CD.0015). Mandatory: Stream Category Code/Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/revenue_stream_category.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/revenue_stream_category_iud.py
"""
import os
import sys
...
VIEW          = "ov_stream_category"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_RSC_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Revenue Stream Category 001"
NAME_UPD      = "AUTOTEST Revenue Stream Category 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Stream Category Code",  "value": CODE,       "kind": "text"},
...
EVID = _ROOT / "tmp" / "revenue_stream_category" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"revenue_stream_category_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_stream_category"

"""Calculation Context - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Calculation Objects, CO.1059). Mandatory: Calc Context Code/Calc Context Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/calculation_context.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/calculation_context_iud.py
"""
import os
import sys
...
VIEW          = "ov_calc_context"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_CALCTX_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Calculation Context 001"
NAME_UPD      = "AUTOTEST Calculation Context 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Calc Context Code",  "value": CODE,       "kind": "text"},
    {"label": "Calc Context Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
]
UPDATE_FIELDS = [
    {"label": "Calc Context Name",  "value": NAME_UPD,   "kind": "text"},
]
...
EVID = _ROOT / "tmp" / "calculation_context" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"calculation_context_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_calc_context"

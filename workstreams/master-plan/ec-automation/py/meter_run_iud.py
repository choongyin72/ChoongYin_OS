"""Meter Run - IUD driver (thin). Reuses the shared engine py/ec_object_iud.py + DbVerify.py.

OV (Stream Objects, CO.0091). Mandatory: Meter Run Code/Meter Run Name/Start Date
(optional dropdowns skipped). Update = Name. Selector map: ec-ui-knowledge/screens/meter_run.md.
Run headed: EC_HEADED=1 py -X utf8 workstreams/master-plan/ec-automation/py/meter_run_iud.py
"""
import os
import sys
...
VIEW          = "ov_meter_run"
CODE          = os.environ.get("EC_CODE", "AUTOTEST_MR_001")
START_DATE    = "2000-01-01"
END_DATE      = START_DATE
NAME          = "AUTOTEST Meter Run 001"
NAME_UPD      = "AUTOTEST Meter Run 001 UPDATED"

INSERT_FIELDS = [
    {"label": "Meter Run Code",  "value": CODE,       "kind": "text"},
    {"label": "Meter Run Name",  "value": NAME,       "kind": "text"},
    {"label": "Start Date",  "value": START_DATE, "kind": "date"},
    {"label": "Type of Taps", "value": "Pipe", "kind": "dropdown"},
...
UPDATE_FIELDS = [
    {"label": "Meter Run Name",  "value": NAME_UPD,   "kind": "text"},
]
...
EVID = _ROOT / "tmp" / "meter_run" / "evidence"
...
def shot(page, label):
    try:
        page.screenshot(path=str(EVID / f"meter_run_{label}.png"))
    except Exception:
        pass
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def _v_ins():
                ...
                assert db.code_present(VIEW, CODE), "not in ov_meter_run"

# SOW - EC Code Object IUD

## Classification
- **Screen:** Configuration > Codes > EC Code Object (BF_CODE **CD.0135**)
- **Type/pattern:** OV (Manage-Object, `manage_object_nav`) - date-effective; plain (no mandatory dropdowns)
- **DB view:** `OV_EC_CODE_OBJECT` (versioned); key `CODE`
- **Delete:** End Date = Start Date -> row leaves `OV_EC_CODE_OBJECT`

## Nav / grid / cells
- **Open:** menu search "EC Code Object" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
- **NO hardcoded field ids** - resolved BY LABEL via T2 `Fill OV * By Label` / `OV Field Id By Label`:
  - **Insert (objectForm):** `Object Code`, `Name`, `Start Date` (mandatory). Optional dropdowns skipped.
  - **Update (updateAttributes):** `Name` (Code read-only; loaded-check via `OV Field Id By Label` on `Object Code`).
  - **Delete (objectdates):** `End Date` = Start Date.

## Test data
- `AUTOTEST_ECO_<timestamp>` unique per run; Start/End = `${TEST_START_DATE}` (2000-01-01). Never touch real rows.

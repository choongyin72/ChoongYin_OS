"""READ-ONLY recon for Choke (CO.0185, OV_CHOKE): DB columns + treeview path +
...
cur.execute("""select column_name,data_type,nullable from all_tab_columns
               where table_name='OV_CHOKE' order by column_id""")
print("=== OV_CHOKE columns ===")
for c in cur.fetchall(): print(f"  {c[0]:<22} {c[1]:<10} null={c[2]}")
# --- treeview path (screen CO.0185) ---
...
def walk(n,path):
    ...
    if n.get("screen")=="CO.0185": hits.append(np)
    for c in n.get("children",[]) or []:
        if isinstance(c,dict): walk(c,np)
for r in data["configuration"]["items"]: walk(r,[])
print("\nTREEVIEW PATH (CO.0185):", " > ".join(hits[0]) if hits else "NOT FOUND")
...
with sync_playwright() as p:
    ...
    print("\nscreen:", ec.open_object_screen(pg,"Choke"))
    ec.click_go(pg)

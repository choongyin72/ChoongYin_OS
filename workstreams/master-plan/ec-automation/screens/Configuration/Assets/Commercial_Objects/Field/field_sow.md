...
existing data; test codes `AUTOTEST_FLD_<timestamp>`; local sandbox, user sysadmin.
...
| Navigator (mandatory before grid loads) | Offshore area + GO |
...
### Test data
Code `AUTOTEST_FLD_<timestamp>` | Name `Field <code>` (+` UPD`) | Start=End `2003-01-01`
...
|---|---|
| Geo Area (reference dd) | `Offshore area` |
...
| Playwright reference run | headless | see `evidence/field_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Commercial_Objects/field_*`,

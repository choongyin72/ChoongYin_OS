...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Company Contact
...
## 1. REQUIREMENT
Automate IUD on the **Company Contact** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_CCON_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_COMPANY_CONTACT` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_COMPANY_CONTACT` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Commercial Objects > Company Contact |
| Screen type | Manage Object (OV) |
| List/grid id | `manageObject:form:T_data` |
| DB view | `OV_COMPANY_CONTACT` |
...
### Test data
Code `AUTOTEST_CCON_<timestamp>` | Name `Company Contact <code>` (+` UPD`) | Start=End `2003-01-01`
...
|---|---|
| Company (reference dd, banner-discovered) | first available option |
...
| Playwright reference run | headless | see `evidence/company_contact_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Commercial_Objects/company_contact_*`,

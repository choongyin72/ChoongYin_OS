"""READ-ONLY recon of the 12 Commercial Objects OV screens. Same method as
financial_objects_recon.py: direct-URL navigation, grid id + first codes,
...
OUT = Path(r"c:/Projects/ChoongYin_OS/tmp/screen_scan/commercial_objects_recon.json")

SCREENS = ["Company", "Customer", "Vendor", "Licence", "MMS Lease", "State Lease",
           "Operator Lease", "Field Group", "Commercial Entity", "Company Contact",
           "Field", "Sub Field"]

scan = json.loads(SCAN.read_text(encoding="utf-8"))
URLMAP = {r["screen"]: r["url"] for r in scan.values()
          if r.get("section") == "Commercial Objects" and r.get("url")}
...
NAVDD_JS = r"""() => {
    const out = [];
    document.querySelectorAll("button[id^='nav:'][id$=':dd_button']").forEach(e => {
        if (e.offsetParent !== null) out.push(e.id.replace(/_button$/, ''));
    });
    return out;
}"""


def dd_options(page, dd):
    try:
...
def recon(page, label, url):
    ...
    navdds = page.evaluate(NAVDD_JS)
    if navdds:
        rec["navDropdowns"] = [{"id": dd, "options": dd_options(page, dd)} for dd in navdds]
    if rec["firstCodes"]:
        try:
...
def discover_views(records):
    ...
    for rec in records:
        ns = norm(rec["screen"])
        m = re.search(r"(?:CLASS_NAME|GROUPMODEL)/([A-Z_0-9]+)", rec.get("url", ""))
...
with sync_playwright() as p:
    ...
    for label in SCREENS:
        ...
        nav = [d["options"][:4] for d in rec.get("navDropdowns", [])]
        print(f"{label:20s} grid={str(rec.get('gridId'))[:34]:34s} codes={len(rec.get('firstCodes') or [])} "
              f"ins={len(rec.get('insertPlan') or [])} mand={mand} nav={nav}")
    ctx.close()
    b.close()
...
print(f"-> {OUT}")
for r in records:
    print(f"  {r['screen']:20s} view={r.get('dbView')} verified={r.get('dbViewVerified')}")

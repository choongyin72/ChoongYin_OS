...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Commercial Entity
...
## 1. REQUIREMENT
Automate IUD on the **Commercial Entity** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_CE_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_COMMERCIAL_ENTITY` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_COMMERCIAL_ENTITY` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Commercial Objects > Commercial Entity |
| Screen type | Manage Object (OV) |
| List/grid id | `manageObject:form:T_data` |
| DB view | `OV_COMMERCIAL_ENTITY` |
| Delete semantics | End Date = Start Date (true delete) |

### DOM reference (rows derived from recon LABELS)
```
INSERT: Code  tab:tabPanel:objectForm:form:G:0:R:2:C:1:in
...
### Test data
Code `AUTOTEST_CE_<timestamp>` | Name `Commercial Entity <code>` (+` UPD`) | Start=End `2003-01-01`
...
| Playwright reference run | headless | see `evidence/commercial_entity_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Commercial_Objects/commercial_entity_*`,

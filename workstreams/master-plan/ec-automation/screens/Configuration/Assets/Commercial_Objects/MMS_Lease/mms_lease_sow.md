...
## 1. REQUIREMENT
Automate IUD on the **MMS Lease** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_MMSL_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_MMS_LEASE` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_MMS_LEASE` | PASS |
...
### Test data
Code `AUTOTEST_MMSL_<timestamp>` | Name `MMS Lease <code>` (+` UPD`) | Start=End `2003-01-01`
...
| Playwright reference run | headless | see `evidence/mms_lease_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Commercial_Objects/mms_lease_*`,

...
existing data; test codes `AUTOTEST_STL_<timestamp>`; local sandbox, user sysadmin.
...
### Test data
Code `AUTOTEST_STL_<timestamp>` | Name `State Lease <code>` (+` UPD`) | Start=End `2003-01-01`
...
| Playwright reference run | headless | see `evidence/state_lease_results.json` |

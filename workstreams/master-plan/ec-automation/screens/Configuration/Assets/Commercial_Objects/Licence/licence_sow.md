...
existing data; test codes `AUTOTEST_LIC_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_LICENCE` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_LICENCE` | PASS |
...
### Test data
Code `AUTOTEST_LIC_<timestamp>` | Name `Licence <code>` (+` UPD`) | Start=End `2003-01-01`
...
| Playwright reference run | headless | see `evidence/licence_results.json` |

...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Company
...
## 1. REQUIREMENT
Automate IUD on the **Company** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_COMP_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_COMPANY` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_COMPANY` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Commercial Objects > Company |
...
### Test data
Code `AUTOTEST_COMP_<timestamp>` | Name `Company <code>` (+` UPD`) | Start=End `2003-01-01`
...
| Playwright reference run | headless | see `evidence/company_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Commercial_Objects/company_*`,

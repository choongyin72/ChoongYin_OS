...
with sync_playwright() as p:
    ...
    tp = eng._field("Target Property")
    base = tp["id"][: -len("_input")]
    page.locator(css(base + "_button")).first.click()
    page.wait_for_timeout(1500)
    opts = page.evaluate(
        "(sel) => Array.from(document.querySelectorAll(sel)).map(e => e.getAttribute('data-item-label'))",
        f"#{base}_panel tr[data-item-label]".replace(":", "\\:"),
    )
    print("Target Property options on click-only (no typing):", opts)

# JOURNAL - Data Extract Set (SP.0049) OV IUD

## 2026-07-26
- **Branch:** `feature/data_extract_set-iud` (own branch, stacked so the shared-engine helpers are present).
...
- **Recon** (`investigation/recon.py`, read-only): DB `CLASS_TYPE=OBJECT` ⇒ OV; treeview
  Configuration > Assets > Data_Mapping_Objects > Data Extract Set. Mandatory Code/Name/Start Date; optional dropdowns skipped.
  Plain Bank-layout OV (single Date+GO nav, mandatory extra beyond Code/Name/Start Date: Owner Class (dropdown)).

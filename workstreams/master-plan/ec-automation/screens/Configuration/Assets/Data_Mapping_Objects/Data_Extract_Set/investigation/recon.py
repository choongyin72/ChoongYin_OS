"""READ-ONLY recon for Berth (SP.0049, OV_SUMMARY_SET): DB columns + treeview path +
...
cur.execute("""select column_name,data_type,nullable from all_tab_columns
               where table_name='OV_SUMMARY_SET' order by column_id""")
print("=== OV_SUMMARY_SET columns ===")
for c in cur.fetchall(): print(f"  {c[0]:<24} {c[1]:<10} null={c[2]}")
# --- row count (real data present?) ---
try:
    cur.execute("select count(*) from OV_SUMMARY_SET"); print("OV_SUMMARY_SET row count:", cur.fetchone()[0])
except Exception as e:
    print("count err:", repr(e)[:120])
# --- treeview path (screen SP.0049) ---
...
def walk(n,path):
    ...
    if n.get("screen")=="SP.0049": hits.append(np)
    for c in n.get("children",[]) or []:
        if isinstance(c,dict): walk(c,np)
for r in data["configuration"]["items"]: walk(r,[])
print("\nTREEVIEW PATH (SP.0049):", " > ".join(hits[0]) if hits else "NOT FOUND")
...
with sync_playwright() as p:
    ...
    print("\nscreen:", ec.open_object_screen(pg,"Data Extract Set"))
    ec.click_go(pg)

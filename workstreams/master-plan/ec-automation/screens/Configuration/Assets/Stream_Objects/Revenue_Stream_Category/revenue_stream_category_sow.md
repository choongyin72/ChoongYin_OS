# SOW - Revenue Stream Category IUD

## Classification
- **Screen:** Configuration > Assets > Stream_Objects > Revenue Stream Category (BF_CODE **CD.0015**)
- **Type/pattern:** OV (Manage-Object, `manage_object_nav`) - date-effective; plain (no mandatory dropdowns)
- **DB view:** `OV_STREAM_CATEGORY` (versioned); key `CODE`
- **Delete:** End Date = Start Date -> row leaves `OV_STREAM_CATEGORY`

## Nav / grid / cells
- **Open:** menu search "Revenue Stream Category" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
- **NO hardcoded field ids** - resolved BY LABEL via T2 `Fill OV * By Label` / `OV Field Id By Label`:
  - **Insert (objectForm):** `Stream Category Code`, `Name`, `Start Date` (mandatory). Optional dropdowns skipped.
  - **Update (updateAttributes):** `Name` (Code read-only; loaded-check via `OV Field Id By Label` on `Stream Category Code`).
  - **Delete (objectdates):** `End Date` = Start Date.

## Test data
- `AUTOTEST_RSC_<timestamp>` unique per run; Start/End = `${TEST_START_DATE}` (2000-01-01). Never touch real rows.

"""READ-ONLY recon for Berth (CD.0015, OV_STREAM_CATEGORY): DB columns + treeview path +
...
cur.execute("""select column_name,data_type,nullable from all_tab_columns
               where table_name='OV_STREAM_CATEGORY' order by column_id""")
print("=== OV_STREAM_CATEGORY columns ===")
for c in cur.fetchall(): print(f"  {c[0]:<24} {c[1]:<10} null={c[2]}")
# --- row count (real data present?) ---
try:
    cur.execute("select count(*) from OV_STREAM_CATEGORY"); print("OV_STREAM_CATEGORY row count:", cur.fetchone()[0])
except Exception as e:
    print("count err:", repr(e)[:120])
# --- treeview path (screen CD.0015) ---
...
def walk(n,path):
    ...
    if n.get("screen")=="CD.0015": hits.append(np)
    for c in n.get("children",[]) or []:
        if isinstance(c,dict): walk(c,np)
for r in data["configuration"]["items"]: walk(r,[])
print("\nTREEVIEW PATH (CD.0015):", " > ".join(hits[0]) if hits else "NOT FOUND")
...
with sync_playwright() as p:
    ...
    print("\nscreen:", ec.open_object_screen(pg,"Revenue Stream Category"))
    ec.click_go(pg)

...
- **Recon** (`investigation/recon.py`, read-only): DB `CLASS_TYPE=OBJECT` ⇒ OV; treeview
  Configuration > Assets > Stream_Objects > Orifice Plate. Mandatory Code/Name/Start Date; optional dropdowns skipped.
  Plain Bank-layout OV (single Date+GO nav, mandatory extras beyond Code/Name/Start Date: Material (dropdown), Diameter [mm], Measurement Temp [deg R]).

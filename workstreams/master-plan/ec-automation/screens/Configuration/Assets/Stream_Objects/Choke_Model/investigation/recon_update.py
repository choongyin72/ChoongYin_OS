...
GRID = "manage_object_nav_nav:form:T_data"; CODE = "AUTOTEST_CHKM_RECON"
def dump(pg, form):
...
with sync_playwright() as p:
    ...
    ec.login(pg, URL, os.environ.get("EC_USER", "sysadmin"), os.environ.get("EC_PASS", "sysadmin")); ec.open_object_screen(pg, "Choke Model"); ec.click_go(pg)
    ec.insertObjectRecord(pg, GRID, [
        {"label":"Choke Model Code","value":CODE,"kind":"text"},
        {"label":"Choke Model Name","value":"AUTOTEST recon","kind":"text"},
    ...
    print("residual after close:", db.count_like("ov_choke_model","AUTOTEST"))
    br.close()

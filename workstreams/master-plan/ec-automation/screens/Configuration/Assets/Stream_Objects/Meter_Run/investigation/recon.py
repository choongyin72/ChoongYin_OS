"""READ-ONLY recon for Berth (CO.0091, OV_METER_RUN): DB columns + treeview path +
...
cur.execute("""select column_name,data_type,nullable from all_tab_columns
               where table_name='OV_METER_RUN' order by column_id""")
print("=== OV_METER_RUN columns ===")
for c in cur.fetchall(): print(f"  {c[0]:<24} {c[1]:<10} null={c[2]}")
# --- row count (real data present?) ---
try:
    cur.execute("select count(*) from OV_METER_RUN"); print("OV_METER_RUN row count:", cur.fetchone()[0])
except Exception as e:
    print("count err:", repr(e)[:120])
# --- treeview path (screen CO.0091) ---
...
def walk(n,path):
    ...
    if n.get("screen")=="CO.0091": hits.append(np)
    for c in n.get("children",[]) or []:
        if isinstance(c,dict): walk(c,np)
for r in data["configuration"]["items"]: walk(r,[])
print("\nTREEVIEW PATH (CO.0091):", " > ".join(hits[0]) if hits else "NOT FOUND")

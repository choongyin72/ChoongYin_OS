...
## Nav / grid / cells
- **Open:** menu search "Meter Run" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
- **NO hardcoded field ids** - resolved BY LABEL via T2 `Fill OV * By Label` / `OV Field Id By Label`:
  - **Insert (objectForm):** `Meter Run Code`, `Meter Run Name`, `Start Date` (mandatory). Optional dropdowns skipped.
  - **Update (updateAttributes):** `Meter Run Name` (Code read-only; loaded-check via `OV Field Id By Label` on `Meter Run Code`).

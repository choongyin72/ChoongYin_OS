...
## Classification
- **Screen:** Configuration > Assets > Revenue_Lists > HCB System (BF_CODE **CD.0097**)
- **Type/pattern:** OV (Manage-Object, `manage_object_nav`) - date-effective; plain (no mandatory dropdowns)
- **DB view:** `OV_BALANCE` (versioned); key `CODE`
- **Delete:** End Date = Start Date -> row leaves `OV_BALANCE`

## Nav / grid / cells
- **Open:** menu search "HCB System" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
## Test data
- `AUTOTEST_HCB_<timestamp>` unique per run; Start/End = `${TEST_START_DATE}` (2000-01-01). Never touch real rows.

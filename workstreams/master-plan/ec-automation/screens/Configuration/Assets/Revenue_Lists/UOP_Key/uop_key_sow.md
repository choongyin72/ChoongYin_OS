...
## Classification
- **Screen:** Configuration > Assets > Revenue_Lists > UOP Key (BF_CODE **CD.0099**)
- **Type/pattern:** OV (Manage-Object, `manage_object_nav`) - date-effective; plain (no mandatory dropdowns)
- **DB view:** `OV_FIN_UOP_DEPR_KEY` (versioned); key `CODE`
- **Delete:** End Date = Start Date -> row leaves `OV_FIN_UOP_DEPR_KEY`

## Nav / grid / cells
- **Open:** menu search "UOP Key" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
## Test data
- `AUTOTEST_UOP_<timestamp>` unique per run; Start/End = `${TEST_START_DATE}` (2000-01-01). Never touch real rows.

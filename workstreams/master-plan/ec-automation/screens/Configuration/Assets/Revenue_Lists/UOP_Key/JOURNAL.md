# JOURNAL - UOP Key (CD.0099) OV IUD

## 2026-07-26
- **Branch:** `feature/uop_key-iud` (own branch, stacked so the shared-engine helpers are present).
...
- **Recon** (`investigation/recon.py`, read-only): DB `CLASS_TYPE=OBJECT` ⇒ OV; treeview
  Configuration > Assets > Revenue_Lists > UOP Key. Mandatory Code/Name/Start Date; optional dropdowns skipped.

# JOURNAL - Input List (CD.0035) OV IUD

## 2026-07-26
- **Branch:** `feature/input_list-iud` (own branch, stacked so the shared-engine helpers are present).
...
- **Recon** (`investigation/recon.py`, read-only): DB `CLASS_TYPE=OBJECT` ⇒ OV; treeview
  Configuration > Assets > Revenue_Lists > Input List. Mandatory Code/Name/Start Date; optional dropdowns skipped.

...
## Classification
- **Screen:** Configuration > Assets > Revenue_Lists > Input List (BF_CODE **CD.0035**)
- **Type/pattern:** OV (Manage-Object, `manage_object_nav`) - date-effective; plain (no mandatory dropdowns)
- **DB view:** `OV_STREAM_ITEM_COLLECTION` (versioned); key `CODE`
- **Delete:** End Date = Start Date -> row leaves `OV_STREAM_ITEM_COLLECTION`

## Nav / grid / cells
- **Open:** menu search "Input List" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
## Test data
- `AUTOTEST_IL_<timestamp>` unique per run; Start/End = `${TEST_START_DATE}` (2000-01-01). Never touch real rows.

...
- **Recon** (`investigation/recon.py`, read-only + tmp/well_hookup/config.json scan): OV-GM (grid
  `manageObject:form:T_data`), navigator cascade Production Unit -> Area -> Facility Class 1. Mandatory Well Hookup Code / Well Hookup Name / Start Date.

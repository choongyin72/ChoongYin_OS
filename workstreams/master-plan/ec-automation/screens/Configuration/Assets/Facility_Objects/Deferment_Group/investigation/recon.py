"""READ-ONLY recon for Berth (CO.0149, OV_DEFERMENT_GROUP): DB columns + treeview path +
...
cur.execute("""select column_name,data_type,nullable from all_tab_columns
               where table_name='OV_DEFERMENT_GROUP' order by column_id""")
print("=== OV_DEFERMENT_GROUP columns ===")
for c in cur.fetchall(): print(f"  {c[0]:<24} {c[1]:<10} null={c[2]}")
# --- row count (real data present?) ---
try:
    cur.execute("select count(*) from OV_DEFERMENT_GROUP"); print("OV_DEFERMENT_GROUP row count:", cur.fetchone()[0])
except Exception as e:
    print("count err:", repr(e)[:120])
# --- treeview path (screen CO.0149) ---
...
def walk(n,path):
    ...
    if n.get("screen")=="CO.0149": hits.append(np)
    for c in n.get("children",[]) or []:
        if isinstance(c,dict): walk(c,np)
for r in data["configuration"]["items"]: walk(r,[])
print("\nTREEVIEW PATH (CO.0149):", " > ".join(hits[0]) if hits else "NOT FOUND")
...
with sync_playwright() as p:
    ...
    print("\nscreen:", ec.open_object_screen(pg,"Deferment Group"))
    ec.click_go(pg)

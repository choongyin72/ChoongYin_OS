# SOW - Config Variable IUD

## Classification
- **Screen:** Configuration > Assets > Calculation_Objects > Config Variable (BF_CODE **IN.0031**)
- **Type/pattern:** OV (Manage-Object, `manage_object_nav`) - date-effective; plain (no mandatory dropdowns)
- **DB view:** `OV_CONFIG_VARIABLE` (versioned); key `CODE`
- **Delete:** End Date = Start Date -> row leaves `OV_CONFIG_VARIABLE`

## Nav / grid / cells
- **Open:** menu search "Config Variable" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
## Test data
- `AUTOTEST_CV_<timestamp>` unique per run; Start/End = `${TEST_START_DATE}` (2000-01-01). Never touch real rows.

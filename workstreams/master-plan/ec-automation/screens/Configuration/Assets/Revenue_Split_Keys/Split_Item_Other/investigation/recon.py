"""READ-ONLY recon for Berth (CD.0017, OV_SPLIT_ITEM_OTHER): DB columns + treeview path +
...
cur.execute("""select column_name,data_type,nullable from all_tab_columns
               where table_name='OV_SPLIT_ITEM_OTHER' order by column_id""")
print("=== OV_SPLIT_ITEM_OTHER columns ===")
for c in cur.fetchall(): print(f"  {c[0]:<24} {c[1]:<10} null={c[2]}")
# --- row count (real data present?) ---
try:
    cur.execute("select count(*) from OV_SPLIT_ITEM_OTHER"); print("OV_SPLIT_ITEM_OTHER row count:", cur.fetchone()[0])
except Exception as e:
    print("count err:", repr(e)[:120])
# --- treeview path (screen CD.0017) ---
...
def walk(n,path):
    ...
    if n.get("screen")=="CD.0017": hits.append(np)
    for c in n.get("children",[]) or []:
        if isinstance(c,dict): walk(c,np)
for r in data["configuration"]["items"]: walk(r,[])
print("\nTREEVIEW PATH (CD.0017):", " > ".join(hits[0]) if hits else "NOT FOUND")
...
with sync_playwright() as p:
    ...
    print("\nscreen:", ec.open_object_screen(pg,"Split Item Other"))
    ec.click_go(pg)

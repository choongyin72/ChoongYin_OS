...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Business Unit
...
## 1. REQUIREMENT
Automate Insert / Update / Delete on the **Business Unit** screen and prove, at DB level,
that EC creates, modifies and truly deletes the record. Constraints: NEVER touch
existing data; all test data prefixed `AUTOTEST_BU_`; environment = local EC
sandbox (`ap-f0a7g341jn6d`), user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_BUSINESS_UNIT` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End Date = Start Date -> gone from grid AND absent in `OV_BUSINESS_UNIT` | PASS |
...
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Basic Objects > Business Unit |
| Screen type | Manage Object (OV) |
| List/grid id | `manage_object_nav_nav:form:T_data` |
| DB view (ground truth) | `OV_BUSINESS_UNIT` |
| Delete semantics | End Date = Start Date (true delete) |

### 2.2 Screen-specific notes
Plain OV, Bank-identical shape (Code/Name/Start Date).
...
| Field | Value |
|---|---|
| Code | `AUTOTEST_BU_<timestamp>` (fresh per run — deleted codes linger in the base table) |
| Name / Name (update) | `Business Unit <code>` / `Business Unit <code> UPD` |
...
| Playwright reference run | headless | see `evidence/business_unit_results.json` |
...
| Deliverable | Where |
|---|---|
| RF suite (maintained test) | `tests/Configuration/Assets/Basic_Objects/business_unit_iud.robot` |
| RF page object | `pageobjects/Configuration/Assets/Basic_Objects/business_unit_page.resource` |
| Playwright reference | `playwright/ec_iud_business_unit.py` (+ `_shared/iud_engine.py`) |

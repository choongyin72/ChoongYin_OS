...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — State
...
## 1. REQUIREMENT
Automate Insert / Update / Delete on the **State** screen and prove, at DB level,
that EC creates, modifies and truly deletes the record. Constraints: NEVER touch
existing data; all test data prefixed `AUTOTEST_ST_`; environment = local EC
sandbox (`ap-f0a7g341jn6d`), user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_STATE` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End Date = Start Date -> gone from grid AND absent in `OV_STATE` | PASS |
...
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Basic Objects > State |
| Screen type | Manage Object (OV) |
| List/grid id | `manage_object_nav_nav:form:T_data` |
| DB view (ground truth) | `OV_STATE` |
| Delete semantics | End Date = Start Date (true delete) |

### 2.2 Screen-specific notes
ROW-SHIFT screen: Master System rows sit ABOVE Code/Name, so State Code/Name are R2/R3 (not R0/R1). Country dropdown (R8) is optional.
...
| Field | Value |
|---|---|
| Code | `AUTOTEST_ST_<timestamp>` (fresh per run — deleted codes linger in the base table) |
| Name / Name (update) | `State <code>` / `State <code> UPD` |
...
- Empty base table meant no update-form recon was possible; update ids were derived from the insert row order and proven live.
...
| Playwright reference run | headless | see `evidence/state_results.json` |
...
| Deliverable | Where |
|---|---|
| RF suite (maintained test) | `tests/Configuration/Assets/Basic_Objects/state_iud.robot` |
| RF page object | `pageobjects/Configuration/Assets/Basic_Objects/state_page.resource` |
| Playwright reference | `playwright/ec_iud_state.py` (+ `_shared/iud_engine.py`) |

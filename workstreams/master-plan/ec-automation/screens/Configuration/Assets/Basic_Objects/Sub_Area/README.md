# Sub Area — Playwright IUD

Insert / Update / Delete automation for the EC **Sub Area** screen
(Configuration → Assets → Basic Objects → Sub Area), implemented in **Playwright** (Python).

Sub Area is a **Manage Object (OV-GM groupmodel)** screen. DELETE = **End Date = Start Date** (zero-length window),
which EC treats as a true delete (object removed from `OV_SUB_AREA`).
...
## Folder
- `playwright/ec_iud_sub_area.py` — thin config over the shared engine (`../_shared/iud_engine.py`)
...
- `sub_area_sow.md` — statement of work / spec

## Equivalent RF suite
`tests/Configuration/Assets/Basic_Objects/sub_area_iud.robot` (the maintained test;

...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Region
...
## 1. REQUIREMENT
Automate Insert / Update / Delete on the **Region** screen and prove, at DB level,
that EC creates, modifies and truly deletes the record. Constraints: NEVER touch
existing data; all test data prefixed `AUTOTEST_REG_`; environment = local EC
sandbox (`ap-f0a7g341jn6d`), user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_REGION` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End Date = Start Date -> gone from grid AND absent in `OV_REGION` | PASS |
...
| Field | Value |
|---|---|
| Code | `AUTOTEST_REG_<timestamp>` (fresh per run — deleted codes linger in the base table) |
| Name / Name (update) | `Region <code>` / `Region <code> UPD` |
...
| Playwright reference run | headless | see `evidence/region_results.json` |
...
| Deliverable | Where |
|---|---|
| RF suite (maintained test) | `tests/Configuration/Assets/Basic_Objects/region_iud.robot` |
| RF page object | `pageobjects/Configuration/Assets/Basic_Objects/region_page.resource` |
| Playwright reference | `playwright/ec_iud_region.py` (+ `_shared/iud_engine.py`) |

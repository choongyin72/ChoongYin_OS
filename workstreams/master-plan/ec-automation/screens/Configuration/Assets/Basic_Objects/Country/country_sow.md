...
existing data; all test data prefixed `AUTOTEST_CTRY_`; environment = local EC
...
Plain OV; 12 insert rows but only Code+Name mandatory; dates at R5/R6.
...
| Field | Value |
|---|---|
| Code | `AUTOTEST_CTRY_<timestamp>` (fresh per run — deleted codes linger in the base table) |
| Name / Name (update) | `Country <code>` / `Country <code> UPD` |
...
| Playwright reference run | headless | see `evidence/country_results.json` |
...
| RF page object | `pageobjects/Configuration/Assets/Basic_Objects/country_page.resource` |
| Playwright reference | `playwright/ec_iud_country.py` (+ `_shared/iud_engine.py`) |

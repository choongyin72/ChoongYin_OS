...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Production Unit
...
## 1. REQUIREMENT
Automate Insert / Update / Delete on the **Production Unit** screen and prove, at DB level,
that EC creates, modifies and truly deletes the record. Constraints: NEVER touch
existing data; all test data prefixed `AUTOTEST_PU_`; environment = local EC
sandbox (`ap-f0a7g341jn6d`), user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_PRODUCTIONUNIT` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End Date = Start Date -> gone from grid AND absent in `OV_PRODUCTIONUNIT` | PASS |
...
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Basic Objects > Production Unit |
| Screen type | Manage Object (OV) |
| List/grid id | `manage_object_nav_nav:form:T_data` |
| DB view (ground truth) | `OV_PRODUCTIONUNIT` |
| Delete semantics | End Date = Start Date (true delete) |

### 2.2 Screen-specific notes
Plain OV. 4 text rows before the dates (Master System Code/Name are optional).
...
| Field | Value |
|---|---|
| Code | `AUTOTEST_PU_<timestamp>` (fresh per run — deleted codes linger in the base table) |
| Name / Name (update) | `Production Unit <code>` / `Production Unit <code> UPD` |
...
- Used as the label-pattern discovery screen (`:C:0:la` labels, {mandatory:true} markers).
...
| Playwright reference run | headless | see `evidence/production_unit_results.json` |
...
| Deliverable | Where |
|---|---|
| RF suite (maintained test) | `tests/Configuration/Assets/Basic_Objects/production_unit_iud.robot` |
| RF page object | `pageobjects/Configuration/Assets/Basic_Objects/production_unit_page.resource` |
| Playwright reference | `playwright/ec_iud_production_unit.py` (+ `_shared/iud_engine.py`) |

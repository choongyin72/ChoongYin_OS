...
Manage-Object (OV) screen, Bank family. Field IDs from recon (scan_ec_screen.py):
  ...
  DELETE  : objectdates:form G:0:R:0:C:3:da_input = EndDate. EC toolbar Delete is disabled;
            ...
            the object entirely from ov_product_group (verified at DB level).
NEVER TOUCH EXISTING DATA. Test data: AUTOTEST_PG_* only.
"""
...
SS_DIR        = str(ROOT / 'docs' / 'EC' / 'screenshots' / 'iud_product_group')
LOG_PATH      = str(ROOT / 'tmp' / 'logs' / 'ec_iud_product_group_final.json')
...
_CODE         = os.environ.get('EC_CODE', 'AUTOTEST_PG_004')
_NUM          = _CODE.split('_')[-1]
TEST_CODE     = _CODE
TEST_NAME     = f'AUTOTEST Product Group {_NUM}'
TEST_NAME_UPD = f'AUTOTEST Product Group {_NUM} UPDATED'
...
def select_row(page, code):
    """Click the row span for a given product group code."""
    span = page.locator(
...
with sync_playwright() as p:
    ...
    print(f'  Screen: {lbl}')
    ss(page, 'product_group_loaded')

    # -- CLEAN STATE / PRE-CLEANUP --------------------------------------------
    print('\n=== CLEAN STATE ===')
    rows0 = get_table_rows(page)
    print(f'  Product Groups: {[r[0] for r in rows0]}')
    ...
    rows0 = get_table_rows(page)
    print(f'  Product Groups now: {[r[0] for r in rows0]}')
    ...
    print(f'  Set End Date={END_DATE} (= Start Date) -> zero-length window -> object removed from ov_product_group.')
    if results.get('insert') == 'PASS':
        ok = select_row(page, TEST_CODE)
        if ok:
            ...
            print(f'  Product Group still in table after delete: {still_visible}')
            if not still_visible:
                print(f'  DELETE PASS: removed (EndDate=StartDate={END_DATE}), gone from ov_product_group')

...
Manage-Object (OV) screen, Bank family. Field IDs from recon (scan_ec_screen.py):
  ...
  DELETE  : objectdates:form G:0:R:0:C:3:da_input = EndDate. EC toolbar Delete is disabled;
            ...
            the object entirely from ov_royalty_owner (verified at DB level).
NEVER TOUCH EXISTING DATA. Test data: AUTOTEST_RO_* only.
"""
...
SS_DIR        = str(ROOT / 'docs' / 'EC' / 'screenshots' / 'iud_royalty_owner')
LOG_PATH      = str(ROOT / 'tmp' / 'logs' / 'ec_iud_royalty_owner_final.json')
...
_CODE         = os.environ.get('EC_CODE', 'AUTOTEST_RO_004')
_NUM          = _CODE.split('_')[-1]
TEST_CODE     = _CODE
TEST_NAME     = f'AUTOTEST Royalty Owner {_NUM}'
TEST_NAME_UPD = f'AUTOTEST Royalty Owner {_NUM} UPDATED'
...
def select_row(page, code):
    """Click the row span for a given royalty owner code."""
    span = page.locator(
...
with sync_playwright() as p:
    ...
    print(f'  Screen: {lbl}')
    ss(page, 'royalty_owner_loaded')

    # -- CLEAN STATE / PRE-CLEANUP --------------------------------------------
    print('\n=== CLEAN STATE ===')
    rows0 = get_table_rows(page)
    print(f'  Royalty Owners: {[r[0] for r in rows0]}')
    ...
    rows0 = get_table_rows(page)
    print(f'  Royalty Owners now: {[r[0] for r in rows0]}')
    ...
    # Hover Insert -> click "New Object" (XPath uses descendant span text)
    ...
    print(f'  Set End Date={END_DATE} (= Start Date) -> zero-length window -> object removed from ov_royalty_owner.')
    if results.get('insert') == 'PASS':
        ok = select_row(page, TEST_CODE)
        if ok:
            ...
            print(f'  Royalty Owner still in table after delete: {still_visible}')
            if not still_visible:
                print(f'  DELETE PASS: removed (EndDate=StartDate={END_DATE}), gone from ov_royalty_owner')

...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Currency
...
## 1. REQUIREMENT
Automate IUD on the **Currency** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_CUR_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_CURRENCY` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_CURRENCY` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Financial Objects > Currency |
...
INSERT: Code  tab:tabPanel:objectForm:form:G:0:R:0:C:1:in
        ...
        Active:               tab:tabPanel:objectForm:form:G:0:R:6:C:1:cb (MANDATORY checkbox)
...
| Playwright reference run | headless | see `evidence/currency_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Financial_Objects/currency_*`,

...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Sales Order
...
## 1. REQUIREMENT
Automate IUD on the **Sales Order** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_SO_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_PRODUCT_SALES_ORDER` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_PRODUCT_SALES_ORDER` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Financial Objects > Sales Order |
| Screen type | Manage Object (OV) |
| List/grid id | `manage_object_nav_nav:form:T_data` |
| DB view | `OV_PRODUCT_SALES_ORDER` |
...
### Test data
Code `AUTOTEST_SO_<timestamp>` | Name `Sales Order <code>` (+` UPD`) | Start=End `2000-01-01`

| Extra mandatory field | Test value |
|---|---|
| Company (reference dropdown, banner-discovered) | first available option |
| Field (reference dropdown, banner-discovered) | first available option |
...
| Playwright reference run | headless | see `evidence/sales_order_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Financial_Objects/sales_order_*`,

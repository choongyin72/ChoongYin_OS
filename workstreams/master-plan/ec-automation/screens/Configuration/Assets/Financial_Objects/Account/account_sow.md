...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Account
...
## 1. REQUIREMENT
Automate IUD on the **Account** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_ACC_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_FIN_ACCOUNT` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_FIN_ACCOUNT` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Financial Objects > Account |
| Screen type | Manage Object (OV) |
| List/grid id | `nav:form:T_data` |
| DB view | `OV_FIN_ACCOUNT` |
...
INSERT: Code  tab:tabPanel:objectForm:form:G:0:R:0:C:1:in
        ...
        GL Account:           tab:tabPanel:objectForm:form:G:0:R:8:C:1:in (MANDATORY text)
...
### Test data
Code `AUTOTEST_ACC_<timestamp>` | Name `Account <code>` (+` UPD`) | Start=End `2000-01-01`

| Extra mandatory field | Test value |
|---|---|
| GL Account | `999999` |
| Cost Object Type (reference dropdown, banner-discovered) | first available option |
...
| Playwright reference run | headless | see `evidence/account_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Financial_Objects/account_*`,

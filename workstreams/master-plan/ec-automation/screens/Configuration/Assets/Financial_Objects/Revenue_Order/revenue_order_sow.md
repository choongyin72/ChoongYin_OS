...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Revenue Order
...
## 1. REQUIREMENT
Automate IUD on the **Revenue Order** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_RO_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_FIN_REVENUE_ORDER` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_FIN_REVENUE_ORDER` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Financial Objects > Revenue Order |
| Screen type | Manage Object (OV) |
| List/grid id | `nav:form:T_data` |
| DB view | `OV_FIN_REVENUE_ORDER` |
...
### Test data
Code `AUTOTEST_RO_<timestamp>` | Name `Revenue Order <code>` (+` UPD`) | Start=End `2000-01-01`
...
| Playwright reference run | headless | see `evidence/revenue_order_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Financial_Objects/revenue_order_*`,

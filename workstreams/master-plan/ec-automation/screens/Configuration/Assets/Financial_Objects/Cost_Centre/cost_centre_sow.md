...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Cost Centre
...
## 1. REQUIREMENT
Automate IUD on the **Cost Centre** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_CC_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_FIN_COST_CENTER` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_FIN_COST_CENTER` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Financial Objects > Cost Centre |
| Screen type | Manage Object (OV) |
| List/grid id | `nav:form:T_data` |
| DB view | `OV_FIN_COST_CENTER` |
...
### Test data
Code `AUTOTEST_CC_<timestamp>` | Name `Cost Centre <code>` (+` UPD`) | Start=End `2000-01-01`
...
| Playwright reference run | headless | see `evidence/cost_centre_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Financial_Objects/cost_centre_*`,

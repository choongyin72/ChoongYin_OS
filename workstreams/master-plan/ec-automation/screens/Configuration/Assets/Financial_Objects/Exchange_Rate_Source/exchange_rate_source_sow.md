...
**Task:** EC Screen Insert/Update/Delete (IUD) Automation — Exchange Rate Source
...
## 1. REQUIREMENT
Automate IUD on the **Exchange Rate Source** screen with DB-level proof. Constraints: NEVER touch
existing data; test codes `AUTOTEST_ERS_<timestamp>`; local sandbox, user sysadmin.

| Operation | Pass condition | Status |
|---|---|---|
| INSERT | AUTOTEST row in grid AND present in `OV_FOREX_SOURCE` | PASS |
| UPDATE | Name change visible in grid row | PASS |
| DELETE | End=Start -> gone from grid AND absent in `OV_FOREX_SOURCE` | PASS |

## 2. DESIGN
| Property | Value |
|---|---|
| Treeview path | Configuration > Assets > Financial Objects > Exchange Rate Source |
| Screen type | Manage Object (OV) |
| List/grid id | `manage_object_nav_nav:form:T_data` |
| DB view | `OV_FOREX_SOURCE` |
...
### Test data
Code `AUTOTEST_ERS_<timestamp>` | Name `Exchange Rate Source <code>` (+` UPD`) | Start=End `2000-01-01`
...
| Playwright reference run | headless | see `evidence/exchange_rate_source_results.json` |

## 5. DELIVERABLES
RF suite + page object under `tests|pageobjects/.../Financial_Objects/exchange_rate_source_*`,

"""Tug Boat - read-only recon (checklist #5). Opens the screen + dumps the New-Object field
...
with sync_playwright() as p:
    ...
    ec.open_object_screen(pg, 'Tug Boat')
    pu = ec.apply_ovgm_navigator(pg)
    print("nav top-parent PU:", pu)
    ec._open_new_object(pg); pg.wait_for_timeout(600)
    print("recon: New-Object form opened (read-only, no Save). View = OV_TUG_BOAT.")
    br.close()

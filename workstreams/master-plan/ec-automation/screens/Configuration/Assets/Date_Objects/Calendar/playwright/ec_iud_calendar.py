"""
EC IUD - Calendar (Configuration > Assets > Date Objects > Calendar, CD.0024).
...
the 7 weekday indicator checkboxes (R:6-R:12) are optional and left at default.
  ...
  DELETE  : objectdates:form G:0:R:0:C:3:da_input = EndDate. The EC-correct delete for a
            ...
            the object entirely from ov_calendar (verified at DB level).
NEVER TOUCH EXISTING DATA. Test data: AUTOTEST_CAL_* only.

This is the freestyle reference flow; the RF suite (calendar_iud.robot) is the
...
BUNDLE        = Path(__file__).resolve().parents[1]     # .../Calendar/
SS_DIR        = str(BUNDLE / 'evidence')
LOG_PATH      = str(BUNDLE / 'evidence' / 'results.json')

HEADED        = os.environ.get('EC_HEADED', '0') == '1'
SLOW_MO       = int(os.environ.get('EC_SLOWMO', '0'))
_CODE         = os.environ.get('EC_CODE', 'AUTOTEST_CAL_001')
_NUM          = _CODE.split('_')[-1]
TEST_CODE     = _CODE
TEST_NAME     = f'AUTOTEST Calendar {_NUM}'
TEST_NAME_UPD = f'AUTOTEST Calendar {_NUM} UPDATED'
...
def ss(page, label):
    ss_index[0] += 1
    name = f'cal_{ss_index[0]:02d}_{label}.png'
...
def click_go(page):
    """Reload the list. Calendar (custom-URL OV) has no GO button -> re-open the screen
...
with sync_playwright() as p:
    ...
    results['navigate'] = 'PASS' if 'Calendar' in lbl else f'FAIL={lbl}'
    print(f'  Screen: {lbl}')
    ss(page, 'loaded')

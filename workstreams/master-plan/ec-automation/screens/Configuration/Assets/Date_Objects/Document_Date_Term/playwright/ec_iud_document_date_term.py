"""
EC IUD - Document Date Term (Configuration > Assets > Date Objects > Document Date Term, CD.0107).
...
(see ../investigation/recon_new_object_form.py + recon_method_dropdown.py):
  ...
  DELETE  : objectdates:form G:0:R:0:C:3:da_input = EndDate. The EC-correct delete for a
            ...
            the object entirely from ov_doc_date_term (verified at DB level).
NEVER TOUCH EXISTING DATA. Test data: AUTOTEST_DDT_* only.

This is the freestyle reference flow; the RF suite (document_date_term_iud.robot) is the
...
BUNDLE        = Path(__file__).resolve().parents[1]     # .../Document_Date_Term/
SS_DIR        = str(BUNDLE / 'evidence')
LOG_PATH      = str(BUNDLE / 'evidence' / 'results.json')

HEADED        = os.environ.get('EC_HEADED', '0') == '1'
SLOW_MO       = int(os.environ.get('EC_SLOWMO', '0'))
_CODE         = os.environ.get('EC_CODE', 'AUTOTEST_DDT_001')
_NUM          = _CODE.split('_')[-1]
TEST_CODE     = _CODE
TEST_NAME     = f'AUTOTEST Doc Date Term {_NUM}'
TEST_NAME_UPD = f'AUTOTEST Doc Date Term {_NUM} UPDATED'
START_DATE    = '2000-01-01'
END_DATE      = '2000-01-01'   # EC DELETE: End Date = Start Date (zero-length window = true delete)
METHOD_LABEL  = 'Set Document Date manually'   # = DOC_DATE_TERM_METHOD MANUAL
OFFSET_VALUE  = '0'
...
def ss(page, label):
    ss_index[0] += 1
    name = f'ddt_{ss_index[0]:02d}_{label}.png'
...
with sync_playwright() as p:
    ...
    results['navigate'] = 'PASS' if 'Document Date Term' in lbl else f'FAIL={lbl}'
    print(f'  Screen: {lbl}')
    ss(page, 'loaded')

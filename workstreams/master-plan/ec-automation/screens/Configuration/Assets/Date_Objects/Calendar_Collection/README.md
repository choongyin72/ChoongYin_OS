...
Configuration > Assets > Date Objects > Calendar Collection. Manage-Object (OV, date-effective)
...
```
# headed (the proof)
EC_HEADLESS=false robot --outputdir results tests/Configuration/Assets/Date_Objects/calendar_collection_iud.robot
# dryrun
...
```
EC_HEADED=1 EC_CODE=AUTOTEST_CC_PW01 py workstreams/master-plan/ec-automation/screens/Configuration/Assets/Date_Objects/Calendar Collection/playwright/ec_iud_calendar_collection.py
```

Read-only recon (never saves):
```
SCREEN="Calendar Collection" py tmp/scripts/scan_ec_screen.py
py workstreams/.../Calendar Collection/investigation/recon_new_object_form.py
py workstreams/.../Calendar Collection/investigation/recon_method_dropdown.py
```

"""
EC IUD - Payment Term (Configuration > Assets > Date Objects > Payment Term, CD.0023).
...
(see ../investigation/recon_new_object_form.py + recon_method_dropdown.py):
  ...
  DELETE  : objectdates:form G:0:R:0:C:3:da_input = EndDate. The EC-correct delete for a
            ...
            the object entirely from ov_payment_term (verified at DB level).
NEVER TOUCH EXISTING DATA. Test data: AUTOTEST_PT_* only.

This is the freestyle reference flow; the RF suite (payment_term_iud.robot) is the
...
BUNDLE        = Path(__file__).resolve().parents[1]     # .../Payment_Term/
SS_DIR        = str(BUNDLE / 'evidence')
LOG_PATH      = str(BUNDLE / 'evidence' / 'results.json')

HEADED        = os.environ.get('EC_HEADED', '0') == '1'
SLOW_MO       = int(os.environ.get('EC_SLOWMO', '0'))
_CODE         = os.environ.get('EC_CODE', 'AUTOTEST_PT_001')
_NUM          = _CODE.split('_')[-1]
TEST_CODE     = _CODE
TEST_NAME     = f'AUTOTEST Payment Term {_NUM}'
TEST_NAME_UPD = f'AUTOTEST Payment Term {_NUM} UPDATED'
START_DATE    = '2000-01-01'
END_DATE      = '2000-01-01'   # EC DELETE: End Date = Start Date (zero-length window = true delete)
METHOD_LABEL  = 'Fixed number of Days'   # = PAYMENT_TERM_METHOD
OFFSET_VALUE  = '30'                      # DAY_VALUE cell (R:8) -- "30 days"
...
INS_METHOD_DD = 'tab:tabPanel:objectForm:form:G:0:R:7:C:1:dd'
INS_OFFSET_ID = 'tab:tabPanel:objectForm:form:G:0:R:8:C:1:in'
...
def ss(page, label):
    ss_index[0] += 1
    name = f'pt_{ss_index[0]:02d}_{label}.png'
...
with sync_playwright() as p:
    ...
    results['navigate'] = 'PASS' if 'Payment Term' in lbl else f'FAIL={lbl}'
    print(f'  Screen: {lbl}')
    ss(page, 'loaded')

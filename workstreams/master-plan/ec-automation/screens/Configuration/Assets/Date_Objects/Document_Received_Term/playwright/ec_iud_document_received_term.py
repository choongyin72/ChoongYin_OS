"""
EC IUD - Document Received Term (Configuration > Assets > Date Objects > Document Received Term, CD.0108).

Manage-Object (OV, date-effective) screen. Field IDs confirmed by DOM recon
(see ../investigation/recon_new_object_form.py + recon_method_dropdown.py):
  INSERT  : objectForm:form G:0:R:0=Code, R:1=Name, R:2:da_input=StartDate,
            R:6:C:1:dd=METHOD (autocomplete dropdown), R:7:C:1:in=OFFSET (mandatory number)
  ...
  DELETE  : objectdates:form G:0:R:0:C:3:da_input = EndDate. The EC-correct delete for a
            ...
            the object entirely from ov_doc_received_term (verified at DB level).
NEVER TOUCH EXISTING DATA. Test data: AUTOTEST_DRT_* only.

This is the freestyle reference flow; the RF suite (document_received_term_iud.robot) is the
...
BUNDLE        = Path(__file__).resolve().parents[1]     # .../Document_Received_Term/
SS_DIR        = str(BUNDLE / 'evidence')
LOG_PATH      = str(BUNDLE / 'evidence' / 'results.json')

HEADED        = os.environ.get('EC_HEADED', '0') == '1'
SLOW_MO       = int(os.environ.get('EC_SLOWMO', '0'))
_CODE         = os.environ.get('EC_CODE', 'AUTOTEST_DRT_001')
_NUM          = _CODE.split('_')[-1]
TEST_CODE     = _CODE
TEST_NAME     = f'AUTOTEST Doc Received Term {_NUM}'
TEST_NAME_UPD = f'AUTOTEST Doc Received Term {_NUM} UPDATED'
START_DATE    = '2000-01-01'
END_DATE      = '2000-01-01'   # EC DELETE: End Date = Start Date (zero-length window = true delete)
METHOD_LABEL  = 'Manual entry'   # = DOC_DATE_TERM_METHOD MANUAL
OFFSET_VALUE  = '0'
...
INS_METHOD_DD = 'tab:tabPanel:objectForm:form:G:0:R:6:C:1:dd'
INS_OFFSET_ID = 'tab:tabPanel:objectForm:form:G:0:R:7:C:1:in'
...
def ss(page, label):
    ss_index[0] += 1
    name = f'drt_{ss_index[0]:02d}_{label}.png'
...
def select_dropdown(page, dd_prefix, label):
    """Open an EC autocomplete dropdown by its chevron, click the row matching data-item-label.
    One re-open retry covers a panel closed by a concurrent partial re-render."""
    btn = f'#{(dd_prefix + "_button").replace(":", "\\:")}'
    row = f"xpath=//*[@id='{dd_prefix}_panel']//tr[normalize-space(@data-item-label)='{label}']"
    page.locator(btn).click()
    try:
        page.locator(row).first.wait_for(state='visible', timeout=6000)
    except Exception:
        page.keyboard.press('Escape')
        page.wait_for_timeout(1500)
        page.locator(btn).click()
        page.locator(row).first.wait_for(state='visible', timeout=10000)
...
with sync_playwright() as p:
    ...
    # -- NAVIGATE -------------------------------------------------------------
    print('\n=== NAVIGATE TO DOCUMENT DATE TERM ===')
    open_screen(page)
    lbl = page.locator('#screenToolbar\\:form\\:screenLabel').text_content(timeout=5000)
    results['navigate'] = 'PASS' if 'Document Received Term' in lbl else f'FAIL={lbl}'
    print(f'  Screen: {lbl}')
    ss(page, 'loaded')
    ...
    select_dropdown(page, INS_METHOD_DD, METHOD_LABEL); print(f'  Method: {METHOD_LABEL}')
    fill(page, INS_OFFSET_ID, OFFSET_VALUE); print(f'  Offset: {OFFSET_VALUE}')

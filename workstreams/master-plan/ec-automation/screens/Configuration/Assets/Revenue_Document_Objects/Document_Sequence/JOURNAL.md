# JOURNAL - Document Sequence (CD.0109) OV IUD

## 2026-07-26
- **Branch:** `feature/document_sequence-iud` (own branch, stacked so the shared-engine helpers are present).
...
- **Recon** (`investigation/recon.py`, read-only): DB `CLASS_TYPE=OBJECT` ⇒ OV; treeview
  Configuration > Assets > Revenue_Document_Objects > Document Sequence. Mandatory Code/Name/Start Date; optional dropdowns skipped.

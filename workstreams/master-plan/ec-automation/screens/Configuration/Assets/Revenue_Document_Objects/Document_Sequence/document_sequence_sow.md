# SOW - Document Sequence IUD

## Classification
- **Screen:** Configuration > Assets > Revenue_Document_Objects > Document Sequence (BF_CODE **CD.0109**)
- **Type/pattern:** OV (Manage-Object, `manage_object_nav`) - date-effective; plain (no mandatory dropdowns)
- **DB view:** `OV_DOC_SEQUENCE` (versioned); key `CODE`
- **Delete:** End Date = Start Date -> row leaves `OV_DOC_SEQUENCE`

## Nav / grid / cells
- **Open:** menu search "Document Sequence" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
## Test data
- `AUTOTEST_DS_<timestamp>` unique per run; Start/End = `${TEST_START_DATE}` (2000-01-01). Never touch real rows.

# JOURNAL - Task Process (CO.0191) OV IUD

## 2026-07-26
- **Branch:** `feature/task_process-iud` (own branch, stacked so the shared-engine helpers are present).
...
- **Recon** (`investigation/recon.py`, read-only): DB `CLASS_TYPE=OBJECT` ⇒ OV; treeview
  Configuration > Task_List > Task Process. Mandatory Code/Name/Start Date; optional dropdowns skipped.

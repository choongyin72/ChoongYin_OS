# IUD Task - Deliverable Checklist (Financial Item Template)

Copied from `docs/IUD-DELIVERABLE-CHECKLIST.md`. Playwright-side items are backed by a fresh live
run (`AUTOTEST_FIT_001`, 2026-08-16); RF-side items are deliberately deferred - see note below.

## Step 0. CHECK-EXISTING-FIRST GATE
- [x] **0a.** No `ec-ui-knowledge/screens/financial-item-template.md` existed before this bundle - now added (#20).
- [x] **0b.** Existing work found (driver + design doc narrative from Phase 4 Pilot 2, never
...
## A. Bundle artifacts
- [x] **1.** `financial_item_template_sow.md`.
- [x] **2.** `README.md`.
- [x] **3.** `JOURNAL.md` (includes the reviewer follow-up disclosure).
- [x] **4.** `playwright/ec_iud_financial_item_template.py` - delegates to `py/financial_item_template_iud.py`.
- [x] **5.** `investigation/` - 4 real recon scripts (menu-path lookup, treeview tooltip check, blank-row diagnosis, DB residual verify).
...
- [x] **13.** DB ground-truth - live driver run + direct DB query, both confirm `AUTOTEST_FIT_001`
      inserted/updated/deleted correctly against `FINANCIAL_ITEM_TEMPLATE`.
- [x] **14.** FULL I-U-D scope - Insert + Update + Delete (physical) all proven, live, via the Playwright driver.
- [x] **15.** Self-clean confirmed - 0 residual for `AUTOTEST_FIT_001` after delete.
...
## E. Knowledge base
- [x] **20.** `ec-ui-knowledge/screens/financial-item-template.md`.

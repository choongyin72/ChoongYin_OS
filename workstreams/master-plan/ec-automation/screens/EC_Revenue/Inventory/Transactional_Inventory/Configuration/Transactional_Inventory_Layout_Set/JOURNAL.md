# JOURNAL - Transactional Inventory Layout Set (IN.0033) OV IUD

## 2026-07-26
- **Branch:** `feature/transactional_inventory_layout_set-iud` (own branch, stacked so the shared-engine helpers are present).
...
- **Recon** (`investigation/recon.py`, read-only): DB `CLASS_TYPE=OBJECT` ⇒ OV; treeview
  EC_Revenue > Inventory > Transactional_Inventory > Configuration > Transactional Inventory Layout Set. Mandatory Code/Name/Start Date; optional dropdowns skipped.

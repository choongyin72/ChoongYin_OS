# JOURNAL - Transactional Inventory Properties (IN.0023) OV IUD

## 2026-07-26
- **Branch:** `feature/transactional_inventory_properties-iud` (own branch, stacked so the shared-engine helpers are present).
...
- **Recon** (`investigation/recon.py`, read-only): DB `CLASS_TYPE=OBJECT` ⇒ OV; treeview
  EC_Revenue > Inventory > Transactional_Inventory > Configuration > Transactional Inventory Properties. Mandatory Code/Name/Start Date; optional dropdowns skipped.

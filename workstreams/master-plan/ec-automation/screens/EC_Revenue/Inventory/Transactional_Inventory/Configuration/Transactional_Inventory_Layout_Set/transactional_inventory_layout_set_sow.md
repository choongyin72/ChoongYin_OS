# SOW - Transactional Inventory Layout Set IUD

## Classification
- **Screen:** EC_Revenue > Inventory > Transactional_Inventory > Configuration > Transactional Inventory Layout Set (BF_CODE **IN.0033**)
- **Type/pattern:** OV (Manage-Object, `manage_object_nav`) - date-effective; plain (no mandatory dropdowns)
- **DB view:** `OV_TRANS_INV_TMPL_SET` (versioned); key `CODE`
- **Delete:** End Date = Start Date -> row leaves `OV_TRANS_INV_TMPL_SET`

## Nav / grid / cells
- **Open:** menu search "Transactional Inventory Layout Set" -> `label.tv-link`. Navigator = single **Date + GO**; grid needs GO.
...
## Test data
- `AUTOTEST_TILS_<timestamp>` unique per run; Start/End = `${TEST_START_DATE}` (2000-01-01). Never touch real rows.

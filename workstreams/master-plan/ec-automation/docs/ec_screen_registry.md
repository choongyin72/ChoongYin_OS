...
| Daily Production Well Status 1 | EC Production (search) > Daily Production Well Status 1 | **N1 daily-status grid** — recon done 2026-06-13, build pending | `PWEL_DAY_STATUS` (+`WELL_HOOKUP_DAY_STATUS`), key (OBJECT_ID, DAYTIME) | **iframe** nav: Date `nav:form:G:0:R:1:C:0:da_input` + PU + (Well Hookup OR Facility Class 1) → GO `button:form:B` | **none — edit-in-place** (rows pre-instantiated, status P; no IUD) | grid id TBD (needs data-bearing PU) | GROUPMODEL/WELL, content in iframe `/com.ec.prod.wr.screens/daily_well_status`; on-screen RECORD STATUS/VALIDATION tabset. Design: `docs/pattern_n1_daily_status_grid_design.md`; recon: `tmp/scripts/wr0001_*` |

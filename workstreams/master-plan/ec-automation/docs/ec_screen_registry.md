...
| Meter | Configuration > Assets > Dispatching Objects > Meter | DEFERRED (slice 2) | `OV_METER` | Business Unit dd + GO | End Date = Start Date | `manageObject:form:T_data` | Mandatory **Delivery Point Name is a POPUP PICKER** (`…:C:1:pin`/`pinB`), not a dd — needs a new T2 gesture; also mandatory Meter Type dd (Entry/Exit/Fuel/Transit) |
| Pipeline | Configuration > Assets > Dispatching Objects > Pipeline | DEFERRED (slice 2) | `OV_PIPELINE` | groupmodel nav + GO | End Date = Start Date | `manageObject:form:T_data` | Full OV-GM with Op/Cp/Geo parent dds (30-row form; Code/Name at R2/R3 after Latitude/Longitude!) |
| Nomination Cycle | Configuration > Assets > Dispatching Objects > Nomination Cycle | DEFERRED (slice 2) | `NOMINATION_CYCLE` (`TV_NOMINATION_CYCLE`) | none | physical | `table:form:T_data` | TV class — reuse table_class T2 |

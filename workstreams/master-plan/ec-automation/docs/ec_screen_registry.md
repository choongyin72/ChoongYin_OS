...
| Sub Daily Production Well Status 1 - by Well | EC Production (search) > Sub Daily Production Well Status 1 - by Well | **N1 sub-daily (NEW pattern)** 🟡 recon build-ready (2026-06-14) — intraday rows break the daily TRUNC key | `PWEL_SUB_DAY_STATUS`, **PK (OBJECT_ID, DAYTIME[+time], SUMMER_TIME)** — hourly grain; needs a datetime-keyed DbVerify (match full timestamp + SUMMER_TIME, not TRUNC) | **non-iframed**; Date `nav:form:G:0…da_input` + **5-dd cascade G1→G5** (G1=Production Unit=**FRMW PU**; G2-G5 empty until G1 picked → PU→Area→FC1→WellHookup→Well) → GO `button:form:B`. Scope: 2024-10-01, FRMW Well 1/2, 24 hourly P rows each, SUMMER_TIME='Y' | **none — edit-in-place** | grid TBD (live-crack post-GO; rows likely = time intervals for the one well) | measured cols = well family (ON_STREAM_HRS, AVG_WH_PRESS…). Open cracks: post-GO grid id + cell↔(hour,col) via edit→diff; datetime-keyed DbVerify variant. Design: `docs/pattern_n1_daily_status_grid_design.md` §Sub-daily; recon: `tmp/scripts/n1_subdaily_*` |

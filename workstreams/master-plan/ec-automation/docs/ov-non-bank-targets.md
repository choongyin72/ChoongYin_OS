...
| Message Group (CO.0236) (2026-07-31, recovered 2026-08-01) | OV-GM, insert PERSISTS but lands in WRONG SCOPE (Functional Area dropdown pick mismatch: requested option 1 'Administration', persisted option 2 'Allocation') - grid never lists it. Suspect code is the SHARED `select_dropdown`/`Fill OV Dropdown By Label` engine used by 22 OV-GM screens; stopped at the 2-attempt limit rather than touching shared code without the shared-file protocol. Self-cleaned (DB End=Start). Originally parked on an unmerged branch (`feature/message-group-iud`); recovered here so the finding is not lost. |
| Facility Class 2 (CO.0021) (2026-08-01) | OV-GM (grid `manageObject:form:T_data`), nav = single OPTIONAL date field + GO (`nav_mode=go_only`), no mandatory dropdown at all. Insert PERSISTS correctly (`PRODUCTION_FACILITY.CLASS_NAME='FCTY_CLASS_2'` confirmed, `OV_FCTY_CLASS_2` view has no RECORD_STATUS/scope filter), but the grid shows "No records found" regardless of nav date. Likely needs a parent-scope/Finder selection this screen's navigator panel doesn't expose. Self-cleaned (DB End=Start, 0 residual). No bundle shipped. |
| Planned Well (CO.0247) (2026-08-01) | OV-GM (grid `manageObject:form:T_data`), 5-level cascade nav; PU/Area/Facility Class 1 = PROVEN P1 values. Insert via the toolbar's "New Object" gesture landed in the WRONG class entirely - `OV_WELL` (`CLASS_NAME='WELL'`), not `OV_PLANNED_WELL` (`CLASS_NAME='PLANNED_WELL'`) - both classes share the SAME base `WELL`/`WELL_VERSION`/`WELL_PERIOD_STATUS` tables. The 2 misplaced rows also had ALL scope columns NULL - orphaned regardless of class. Self-cleaned via direct SQL DELETE (child-first: `WELL_PERIOD_STATUS` -> `WELL_VERSION` -> `WELL`, scoped by OBJECT_ID, read in full before delete). 0 residual confirmed. No bundle shipped. |
| Price Index (CO.3009) (2026-08-02) | OV-GM, single Business Unit nav (PROVEN value "Royalty Canada", 23/52 open rows confirmed via `find_populated_scope.py`). This screen's New-Object form has 2 dropdowns in sequence (Frequency, then Business Unit/`parent_dd`). Filled back-to-back, the SECOND dropdown (Business Unit) silently persisted the WRONG value (`SS1_BU` instead of the intended `Royalty Canada`/`ROYALTY_CA`) - reproduced 3 times, including with an explicit 0.8-1s wait added after each dropdown fill (both in a local Playwright driver wrapper AND the RF T3 keyword) which did NOT fix it, ruling out a pure timing race. The visible input DOES show "Royalty Canada" immediately after fill (confirmed by reading `el.value` in-browser), so the widget's HIDDEN bound value is likely not syncing on the second dropdown - a deeper widget-state issue than a race condition. Stopped at the 3rd attempt (past the 2-attempt limit) rather than keep guess-patching a shared-engine-adjacent behavior. Self-cleaned (DB End=Start) x2 for both reproduction attempts, 0 residual. No bundle shipped. NOTE: this may also affect Area's previously-"validated 7/7" `parent_dd` mechanism if that screen's form ever gains a second dropdown before the parent_dd one - Area's validation had only the ONE dropdown, not two in sequence, so it never exercised this path. |

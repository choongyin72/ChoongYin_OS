...
| CO.1033 | Production Day Table | OV_PRODUCTION_DAY | System | (P) Insert toolbar icon does nothing (3rd screen with this shared toolbar pattern) - see Parked table |
...
| Production Day Table (CO.1033) | **RE-ATTEMPTED 2026-08-02, STILL PARKED - never got past Insert, so the original physical-delete concern was never re-tested.** Confirmed this IS a genuine TV-style inline-editable grid (`production_day:form:T_data`, 27 real rows: Object Code/Time Zone/Start Date/End Date/Name/Default/multiple offset columns/Description), matching the classic Language-exemplar pattern. **New blocker found: the Insert toolbar icon does nothing** - clicking it directly produces no new blank row, no DB change, no console error (checked JS console explicitly - clean, no exceptions). Hovering it shows a menu with "PRODUCTION DAYS" as the first item, but that text is NOT a clickable menu link (confirmed via Playwright visibility wait failure) - it is a tooltip on the icon itself, same as the equivalent "CONSTANT STANDARD" tooltip found on Constant Standard's Insert icon. **This is now the 3rd screen in this batch (with Constant Standard and Stream Item) to hit a toolbar-interaction dead end on Insert** - all three share the identical "System of Measurement override" toolbar submenu shape. Given the accumulating pattern, recommend a single dedicated investigation into this shared toolbar mechanism (network/AJAX capture during a successful manual insert, or asking someone with EC domain knowledge how these screens are used in practice) rather than continuing to retry slightly different Playwright selectors per screen. 0 residual confirmed (`AUTOTEST_PDT%`, none), nothing to self-clean - no Save was ever reached. |

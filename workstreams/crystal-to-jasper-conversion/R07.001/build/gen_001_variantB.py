...
OUT = B + r"\output\R07_001_variantB.jrxml"
...
def page_elements(p):
    ...
    for x, y, w, h, col, lw in fills:
        ...
        if ny1 - ny >= 4 and any(abs(e - ny1) < 0.01 for e in by):
            ny1 -= 1
        # VARIANT B ONLY: also inset the LEFT by 1pt, so the band's grey left border line is
        # visible beside the purple instead of underneath it. The reference insets 0.5pt
        # (border 23.15, fill 23.65); 0.5 is unreachable on an integer grid, so this is 1pt -
        # twice the reference's width. Variant A (gen_001.py) keeps the fill flush at 0pt.
        # Both are built so the owner can choose by looking, rather than me guessing again.
        if nx1 - nx >= 6 and any(abs(e - nx) < 0.01 for e in bx):
            nx += 1
    ...
    for x, y, w, h, col, lw in rules:
        out.append(f'<element kind="rectangle" x="{x}" y="{y}" width="{w}" height="0" '
                   f'mode="Transparent">'
                   f'<pen lineWidth="{lw:.1f}" lineColor="{col}"/></element>')
    for x, y, w, h, col, lw in vrules:
        out.append(f'<element kind="rectangle" x="{x}" y="{y}" width="0" height="{h}" '
                   f'mode="Transparent">'
                   f'<pen lineWidth="{lw:.1f}" lineColor="{col}"/></element>')
